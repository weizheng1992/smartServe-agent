...
async function seedMallData() {
  console.log(
    "🌱 [Seed Mall Data] 开始初始化商场全量商品库、运营经理归属与订单销售记录...",
  );
  ...
  try {
    const existingUsers = await drizzle
      .select()
      .from(users)
      .where(eq(users.email, defaultManagerEmail))
      .limit(1);

    if (existingUsers.length > 0) {
      managerId = existingUsers[0].id;
      console.log(
        `[Seed Mall Data] 绑定现有运营经理: ${defaultManagerEmail} (ID: ${managerId})`,
      );
    } else {
      ...
      console.log(
        `[Seed Mall Data] 创建新运营经理: ${defaultManagerEmail} (ID: ${managerId})`,
      );
    }
  } catch (err) {
  ...
  const mallProducts = [
    ...
    },
    {
      ...
      businessId: "nike",
      managerId: "other_manager_alex", // 其他人负责的商品 (用于测试数据隔离)
  ...
  //
  // 销量设计预期：
  // 1) 运动长筒袜 (prod_nike_socks_pack): 卖出 150 盒 -> 销量第一 (150 件), GMV = 14,850 元, 利润 = 11,850 元
  // 2) 经典飞马跑鞋 (prod_nike_pegasus): 卖出 30 双 -> 销量第二 (30 件), GMV = 26,970 元 (GMV 第一!), 利润 = 16,470 元
  // 3) 顶级竞速跑鞋 (prod_nike_vaporfly): 卖出 18 双 -> 销量第三 (18 件), GMV = 28,782 元, 利润 = 19,800 元 (净利润第一!)
  // 4) 运动连帽衫 (prod_nike_tech_fleece): 卖出 10 件 -> 销量第四 (10 件), GMV = 7,990 元, 利润 = 5,190 元
