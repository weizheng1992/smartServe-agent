...
async function main() {
  ...
  console.log(`[Seed Orders] 用户已就绪: ID = ${user.id}, Email = ${user.email}`);
  ...
  for (const merchant of merchants) {
    ...
    for (const ord of merchant.orders) {
      ...
      for (const it of ord.items) {
        await db.execute(
          ...
          [it.productId, merchant.businessId, it.name, `${merchant.name} - ${it.name}`, it.price],
        );
      }
      ...
      if (ord.status !== 'shipped') {
        await db.execute(`UPDATE orders SET status = $1 WHERE order_id = $2`, [ord.status, ord.orderId]);
      }
    ...
    console.log(`  🔍 验证该商户会话查单结果: 共查出 ${listRes.orders?.length || 0} 笔订单`);
  }
