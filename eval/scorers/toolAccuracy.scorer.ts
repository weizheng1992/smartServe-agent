export default function (output: string, context: any) {
  try {
    ...
    const expectedToolsArray = Array.isArray(expectedTools) ? expectedTools : [expectedTools];

    const matched = expectedToolsArray.every((t: string) => calledTools.includes(t));
    if (matched) {
      return {
        pass: true,
        score: 1.0,
