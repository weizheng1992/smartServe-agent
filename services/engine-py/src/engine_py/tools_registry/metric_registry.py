...
METRIC_SEMANTIC_REGISTRY: dict[str, dict[str, Any]] = {
    ...
    """,
        ...
        "synonyms": ["销量", "走量", "爆款", "出货量", "卖得多", "单量最多", "件数最多", "件数", "畅销款", "销量排行", "销量榜", "销量top"],
        "conflictGroup": ["sales_performance_ranking"],
    ...
    """,
        ...
        "synonyms": ["最赚钱", "利润最高", "利润最大", "毛利", "毛利润", "净利润", "赚得多", "净赚", "收益最高"],
        "conflictGroup": ["sales_performance_ranking"],
    ...
    """,
        ...
        "synonyms": ["毛利率", "利润率", "溢价最高", "溢价空间", "性价比最高", "赚钱效率", "回报率"],
        "conflictGroup": ["sales_performance_ranking"],
    ...
    "promo_orders": {
        ...
        "synonyms": ["核销订单", "活动核销", "活动使用情况", "核销了多少", "各活动的核销情况"],
    ...
    "promo_discount_total": {
        ...
        "icon": "💸",
        "aliases": ["promo_cost"],
        "synonyms": ["优惠总额", "活动成本", "优惠了多少钱", "补贴了多少", "活动花了多少", "补贴花了多少", "活动补贴"],
        "conflictGroup": ["promotion_family"],
    ...
    "gmv_trend": {
        ...
        "icon": "📈",
        "aliases": ["gmv_trend"],
        "synonyms": ["GMV 趋势", "销售额趋势", "销售额走势", "GMV走势", "GMV趋势", "每日销售额", "GMV 走势", "趋势"],
        "conflictGroup": [],
        "sampleQueries": ["近 30 天 GMV 趋势", "每天的销售额走势", "近30天GMV趋势"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.95,
    },
    "volume_trend": {
        "key": "volume_trend",
        "label": "销量趋势",
        "description": "销量(件)折线:默认近 30 天按日;问「近 N 个月/几个月的销量」自动切自然月粒度。",
        "domain": "sales",
        "sourceTables": ["merchant_orders", "merchant_order_items"],
        "expression": "SUM(quantity) GROUP BY day|month",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径(排除退款/取消);月粒度含当月,自然月对齐"],
        "direction": "ASC",
        "unit": "件",
        "icon": "📈",
        "aliases": ["volume_trend"],
        "synonyms": ["销量趋势", "销量走势", "月销量", "每月销量", "月度销量", "按月销量", "个月的销量", "月销趋势", "出货趋势", "按月统计"],
        "conflictGroup": [],
        "sampleQueries": ["近3个月的销量", "每月销量趋势", "几个月的销量怎么样"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
    ...
    "order_overview": {
        ...
        "synonyms": ["订单概览", "这几笔订单", "选中订单", "平均金额", "合计金额", "这几单", "勾选订单的统计", "选中订单的概览", "所选订单", "订单对比", "两单对比", "两个订单对比", "这两笔订单对比", "订单比较", "这两笔订单", "勾选的订单", "订单详情", "这单详情", "单笔订单"],
        "conflictGroup": [],
    ...
    # ---- 阶段⑥对话出口族(用户实弹诉求:更多可问意图;登记即全链路) ----
    "aov": {
        "key": "aov",
        "label": "客单价",
        "description": "有效订单(排除退款/取消)的平均实付金额;支持时间窗。",
        "domain": "sales",
        "sourceTables": ["merchant_orders"],
        "expression": "AVG(total_amount) FILTER status NOT IN (REFUNDED, CANCELLED)",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径,与 GMV 榜同源"],
        "direction": "DESC",
        "unit": "元",
        "icon": "🧾",
        "aliases": ["average_order_value"],
        "synonyms": ["客单价", "平均每单", "平均订单金额", "每单平均"],
        "conflictGroup": [],
        "sampleQueries": ["本月客单价多少", "平均每单多少钱"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
    ...
    "order_count": {
        "key": "order_count",
        "label": "订单量",
        "description": "有效订单笔数(排除退款/取消);支持时间窗。",
        "domain": "sales",
        "sourceTables": ["merchant_orders"],
        "expression": "COUNT(*) FILTER status NOT IN (REFUNDED, CANCELLED)",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径,与 GMV 榜同源"],
        "direction": "DESC",
        "unit": "单",
        "icon": "📦",
        "aliases": ["order_volume"],
        "synonyms": ["订单量", "订单数", "成交笔数", "下了多少单"],
        "conflictGroup": [],
        "sampleQueries": ["本月订单量多少", "上周成交笔数"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.92,
    },
    "review_good": {
        "key": "review_good",
        "label": "好评榜",
        "description": "商品好评(评分 ≥ 4)条数排行;商户真实评价。",
        "domain": "review",
        "sourceTables": ["merchant_product_reviews"],
        "expression": "COUNT(*) FILTER rating >= 4 GROUP BY spu_id",
        "sqlTemplate": "merchant_db",
        "businessRules": ["好评口径 = rating ≥ 4(与差评 rating ≤ 2 对偶)"],
        "direction": "DESC",
        "unit": "条",
        "icon": "🌟",
        "aliases": ["positive_review_rank"],
        "synonyms": ["好评榜", "好评最多的商品", "好评数排行"],
        "conflictGroup": [],
        "sampleQueries": ["好评最多的商品 Top5", "看看好评榜"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.92,
    },
    "zero_sales": {
        "key": "zero_sales",
        "label": "滞销零销量商品",
        "description": "在售但一单未成交的商品,按剩余库存降序(压货风险交叉视角)。",
        "domain": "inventory",
        "sourceTables": ["merchant_spus", "merchant_order_items"],
        "expression": "NOT EXISTS(order_items) ORDER BY stock DESC",
        "sqlTemplate": "merchant_db",
        "businessRules": ["只列在售款;库存降序暴露压货风险"],
        "direction": "DESC",
        "unit": "件",
        "icon": "🕳️",
        "aliases": ["unsold_products"],
        "synonyms": ["滞销零销量", "零销量商品", "零销量", "没销量的商品", "一单没卖的商品"],
        "conflictGroup": [],
        "sampleQueries": ["哪些商品零销量", "滞销零销量商品有哪些"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "category_gmv_top": {
        "key": "category_gmv_top",
        "label": "品类GMV榜",
        "description": "按品类聚合的有效成交 GMV 排行;支持时间窗。",
        "domain": "sales",
        "sourceTables": ["merchant_spus", "merchant_order_items", "merchant_orders"],
        "expression": "SUM(quantity*price) GROUP BY category",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径;仅统计在售款"],
        "direction": "DESC",
        "unit": "元",
        "icon": "🏷️",
        "aliases": ["category_gmv_rank"],
        "synonyms": ["品类GMV榜", "品类GMV排行", "品类销售额排行", "各品类销售额"],
        "conflictGroup": [],
        "sampleQueries": ["品类GMV排行", "各品类销售额对比"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "customer_spend_top": {
        "key": "customer_spend_top",
        "label": "客户消费榜",
        "description": "客户累计消费金额排行(有效成交口径);支持时间窗。",
        "domain": "customer",
        "sourceTables": ["merchant_orders", "merchant_customers"],
        "expression": "SUM(total_amount) GROUP BY customer ORDER BY DESC",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径;展示客户名与手机号"],
        "direction": "DESC",
        "unit": "元",
        "icon": "👑",
        "aliases": ["customer_spend_rank"],
        "synonyms": ["客户消费榜", "客户消费排行", "消费最高的客户", "花钱最多的客户"],
        "conflictGroup": [],
        "sampleQueries": ["消费最高的客户 Top5", "客户消费排行"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    # ---- 阶段⑦客户族 + 场景包(grill 设计定稿:客户3 + 场景2 + 环比 + 走势 + 货值) ----
    "customer_spend_stats": {
        "key": "customer_spend_stats",
        "label": "客户消费统计",
        "description": "指定客户的订单数/累计消费/客单价/最近下单;有效成交口径。",
        "domain": "customer",
        "sourceTables": ["merchant_customers", "merchant_orders"],
        "expression": "COUNT/SUM/AVG(total_amount) FILTER customer = ANY(entities)",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径;客户实体必传(逐字绑定或反问)"],
        "direction": "DESC",
        "unit": "元",
        "icon": "🧾",
        "aliases": ["customer_spend"],
        "synonyms": ["消费统计", "消费情况", "消费总额", "花了多少"],
        "conflictGroup": [],
        "sampleQueries": ["张伟的消费统计", "张伟花了多少"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "customer_coupons": {
        "key": "customer_coupons",
        "label": "客户优惠券",
        "description": "指定客户名下的优惠券列表(券名/面额/状态/领取与核销时间)。",
        "domain": "customer",
        "sourceTables": ["user_coupons", "promotions"],
        "expression": "JOIN promotions FILTER user_id = ANY(entities)",
        "sqlTemplate": "merchant_db",
        "businessRules": ["含已用与未用;按领取时间倒序"],
        "direction": "DESC",
        "unit": "张",
        "icon": "🎟️",
        "aliases": ["customer_coupon_list"],
        "synonyms": ["优惠券", "优惠券列表", "持有哪些券", "领了哪些券"],
        "conflictGroup": [],
        "sampleQueries": ["张伟的优惠券", "张伟领了哪些券"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "customer_profile": {
        ...
        "label": "用户画像",
        "description": "指定客户画像:会员级/注册/累计消费/订单数/客单价/最近下单/最爱品类/领券用券数。",
        "domain": "customer",
        "sourceTables": ["merchant_customers", "merchant_orders", "user_coupons"],
        "expression": "聚合 + 最爱品类子查询 + 券计数子查询",
        "sqlTemplate": "merchant_db",
        "businessRules": ["宽表单行;消费均为有效成交口径"],
        "direction": "DESC",
        "unit": "人",
        "icon": "🧑‍💼",
        "aliases": ["customer_persona"],
        "synonyms": ["用户画像", "客户画像", "画像"],
        "conflictGroup": [],
        "sampleQueries": ["张伟的用户画像", "看看张伟的客户画像"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "customer_panorama": {
        "key": "customer_panorama",
        "label": "客户全景(场景包)",
        "description": "一次问出客户三节:画像 + 优惠券 + 最近订单;编排层展开为多卡。",
        "domain": "customer",
        "sourceTables": ["merchant_customers"],
        "expression": "scenario: customer_profile + customer_coupons + customer_orders",
        "sqlTemplate": "",
        "businessRules": ["场景包由编排层展开多帧;客户实体必传"],
        "direction": "DESC",
        "unit": "人",
        "icon": "🗂️",
        "aliases": ["customer_full_view"],
        "synonyms": ["客户全景", "客户全景看下", "客户全貌", "全貌"],
        "conflictGroup": [],
        "sampleQueries": ["张伟的客户全景", "看看张伟的全貌"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "biz_overview": {
        "key": "biz_overview",
        "label": "经营概览(场景包)",
        "description": "一次问出经营五节:GMV/订单量/客单价/会话量/退款率;编排层展开为多卡。",
        "domain": "sales",
        "sourceTables": ["merchant_orders", "session_metrics", "merchant_order_items"],
        "expression": "scenario: gmv + order_count + aov + session_volume + refund_rate",
        "sqlTemplate": "",
        "businessRules": ["场景包由编排层展开多帧;每节独立口径注记"],
        "direction": "DESC",
        "unit": "元",
        "icon": "📊",
        "aliases": ["business_overview"],
        "synonyms": ["经营概览", "经营怎么样", "经营情况", "生意怎么样", "大盘"],
        "conflictGroup": [],
        "sampleQueries": ["经营概览", "最近生意怎么样"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "gmv_mom": {
        "key": "gmv_mom",
        "label": "GMV 环比",
        "description": "本月 vs 上月 GMV 对比与环比百分比;自然月对齐。",
        "domain": "sales",
        "sourceTables": ["merchant_orders", "merchant_order_items"],
        "expression": "本月子查询 / 上月子查询 + 环比",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径;上月为 0 时环比置空(不编造)"],
        "direction": "DESC",
        "unit": "元",
        "icon": "↔️",
        "aliases": ["gmv_mom_compare"],
        "synonyms": ["环比", "GMV环比", "比上个月", "销售环比"],
        "conflictGroup": [],
        "sampleQueries": ["本月比上个月卖得怎么样", "GMV环比"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "orders_trend": {
        "key": "orders_trend",
        "label": "订单量趋势",
        "description": "订单量(单)折线:默认近 30 天按日;「近 N 个月」切自然月粒度。",
        "domain": "sales",
        "sourceTables": ["merchant_orders"],
        "expression": "COUNT(DISTINCT order_id) GROUP BY day|month",
        "sqlTemplate": "merchant_db",
        "businessRules": ["有效成交口径;粒度规则同销量趋势"],
        "direction": "ASC",
        "unit": "单",
        "icon": "📉",
        "aliases": ["order_count_trend"],
        "synonyms": ["订单量趋势", "订单走势", "每日订单", "每天订单数", "每日订单量"],
        "conflictGroup": [],
        "sampleQueries": ["每日订单量走势", "近3个月订单量趋势"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
    "stock_value": {
        "key": "stock_value",
        "label": "库存价值排行",
        "description": "在售商品库存货值(Σ 库存×售价)排行;压货资金视角。",
        "domain": "inventory",
        "sourceTables": ["merchant_spus", "merchant_skus"],
        "expression": "SUM(stock*price) GROUP BY spu ORDER BY DESC",
        "sqlTemplate": "merchant_db",
        "businessRules": ["仅统计在售款;展示品类与库存件数"],
        "direction": "DESC",
        "unit": "元",
        "icon": "💰",
        "aliases": ["inventory_value"],
        "synonyms": ["库存价值", "库存金额", "库存货值", "压货资金", "压的货值最大", "货值最大"],
        "conflictGroup": [],
    ...
    "customer_spend_trend": {
        "key": "customer_spend_trend",
        "label": "客户消费趋势",
        "description": "指定客户的消费金额折线:默认近 30 天按日,「近 N 个月」切自然月;有效成交口径。",
        "domain": "customer",
        "sourceTables": ["merchant_customers", "merchant_orders"],
        "expression": "SUM(total_amount) FILTER customer = ANY(entities) GROUP BY day|month",
        "sqlTemplate": "merchant_db",
        "businessRules": ["客户实体必传;只走订单表防明细行放大金额"],
        "direction": "ASC",
        "unit": "元",
        "icon": "📈",
        "aliases": ["customer_spend_line"],
        "synonyms": ["消费记录", "消费趋势", "消费折线", "消费走势"],
        "conflictGroup": [],
        "sampleQueries": ["张伟的消费记录折线图", "张伟近3个月的消费趋势"],
        "availableDimensions": [],
        "permissionTag": "sales_viewer",
        "verifiedConfidence": 0.9,
    },
