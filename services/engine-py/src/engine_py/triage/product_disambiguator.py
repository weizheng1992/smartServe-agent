...
置信居中/低置信/失败则交由调用方出商品选择卡片问用户;「都不像」
(confidence < 0.2,2026-09-14 坏例探测 S01/S10 实证补的出口)交调用方
出引导文案 —— 逼用户从无关选项里点选既伤体验也污染 badcase 数据。
任何失败绝不炸会话主链路 —— 最坏多问一次(与 vision 同哲学)。
...
# 「都不像」底线(2026-09-14 坏例探测 S01/S10 实证):低于此值模型判定图片与
# 任何候选都无关 —— 逼用户从无关选项里点选既伤体验也污染数据,改出「未找到
# 相关商品」引导。与 prompt 规则 3 的「都对不上给 0.2 以下」同一锚点。
CONFIDENCE_NO_MATCH = 0.2
...
2. confidence 表示把握:品类+特征(颜色/款式)都能对上给 0.8 以上;仅品类相似给 0.4~0.7;
   图片内容与所有候选都无关(如风景照、别家商品)给 0.2 以下;
...
async def disambiguate_product(
    ...
    *,
    ...
    - {"status": "no_match", "candidates": [...]}        模型判定图片与所有候选都无关
    - {"status": "ambiguous", "candidates": [...]}       多候选/置信居中/模型失败/疑似幻觉
    """
    ...
    # 「都不像」出口(2026-09-14 坏例探测):模型按规则给底线以下置信 = 图片与
    # 候选全无关联,交由接线层出引导文案而非逼选卡片;疑似幻觉(高置信但不在
    # 候选集)不落此分支 —— 那说明模型看出了些什么,出卡片让人裁更稳。
    if confidence < CONFIDENCE_NO_MATCH:
