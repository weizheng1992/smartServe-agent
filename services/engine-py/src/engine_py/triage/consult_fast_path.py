...
async def answer_consult_from_rag(
    ...
    prompt = (
        ...
        "[RECENT CONVERSATION — UNVERIFIED MEMORY, NOT A FACT SOURCE]:\n"
        ...
        "products, models, prices, or conditions.\n"
        "2. NEVER copy product names, models, specs, or prices from [RECENT CONVERSATION] — it is "
        "unverified memory and may contain outdated or wrong claims. Every product and price in "
        "your reply MUST be quoted verbatim from [STORE POLICY EXCERPTS]; anything not found "
        "there is NOT in the current store catalog, even if the customer or a previous reply "
        "mentioned it.\n"
        "3. If the customer asks to compare several items but the excerpts contain fewer items, "
        "present ONLY the items found in the excerpts and honestly say the other items are not "
        "in the current store catalog. Do NOT fill the table with items from memory or invention.\n"
        "4. If the excerpts do not cover the question, honestly say in Chinese that you need to "
        "double-check with a colleague, and invite the customer to reply 「转人工」 for human help.\n"
        "5. Be concise, warm and professional. Quote concrete numbers/conditions from the excerpts "
        "when present (e.g. return window in days, tag/packaging requirements, who pays shipping).\n"
        f'6. Refer to the store strictly as "{brand_name}". Reply fully in Chinese.\n'
        "7. You may close with a light offer to help further (e.g. providing an order ID to process "
        "a return), but do NOT claim any action has already been executed.\n"
        "8. ROUTING VETO (intent arbitration duty): first check what the customer actually wants. "
