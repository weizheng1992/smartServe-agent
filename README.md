...
> 面向 SaaS 平台运维与全局多租户管理者，提供跨租户统一监控、11 大管控模块(含全局大盘)、全链路决策 Trace、会话实时接管与 HITL 审批流。

![SaaS 控制平面中台](public/admin.png)

- **11 大管控模块体系**：全局大盘、租户配置、决策拓扑、风控审批、多租户会话与接管、Contextual RAG 知识库、技能与 SOP 插件、工具沙箱、双层画像与记忆、全链路可观测性、计量计费。
- **全局指标大盘**：实时透视活跃租户数（近 7 天有会话落账）、待处理 HITL 审批工单（含最老积压时长）、Token 累计消耗/成本折算、近 24h LLM 调用与人工接管会话数；AI 自动解决率（Autopilot Rate）由 `session_metrics` 库内真算（`resolved_auto` / 总数），全部指标与 SQL 可对账，无写死宣传数字。
...
- **多规格货架与品类切换**：9 大品类 30 SPU / 92 SKU 多规格目录（户外机能 / 潮流T恤 / 下装裤类 / 潮流鞋靴 / 背包收纳 / 露营装备 / 衬衫 / 配饰 / 运动配件），首页品类 tab 带计数徽标一键过滤，选规格加购 → 购物车结算 → 订单中心全链路可走通；热销排行按商户真订单聚合（排除退款单，ADR-0002）。
...
| **质量保障与自动化测试**                 | 少量零散单元测试                                       | **全自动化测试流水线**：pytest 契约套件（密封 testcontainers PG+Redis，121 例：HTTP/SSE/socket.io 契约 + AST 沙箱）+ engine 638 例（意图仲裁/视觉消歧/记忆/发件箱对账/多意图不打断）、Playwright 真实浏览器 E2E 与 Promptfoo Python Provider 评估全覆盖。                   |
...
│   │   └── tests/                      # pytest 套件 638 例 (密封 PG 夹具: 意图仲裁/视觉消歧/记忆/发件箱对账/多意图不打断/scheduler 等)
│   │
