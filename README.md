...
│   │   │   └── system.test.ts       # 仿真端到端集成测试
│   │   ├── test-single.ts           # 单次极速本地测试脚本
│   │   ├── test-two-turns.ts        # 🆕 E2E 跨多轮会话记忆断档自愈测试脚本
│   │   ├── test-list-orders.ts      # 🆕 E2E 顾客名下订单列表物理工具检索测试脚本
│   │   ├── inspect-rag.ts           # RAG 切片高画质调试脚本
│   │   └── test-hello.ts            # 打招呼极速通道直达测试脚本
│   │
...
### 6.2 🆕 多轮对话上下文健忘症 E2E 验证 (Turn 1 -> Turn 2)
```bash
