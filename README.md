...
> 💡 **版本与架构演进说明**：
>
> - **v3 (当前版本)**：后端整体 Python 化（2026-09 完成）——NestJS 网关翻转为 **FastAPI**（`services/gateway-py`），决策引擎移植为 **Python LangGraph**（`services/engine-py`），数据库所有权由 Drizzle 翻转为 **SQLAlchemy + Alembic**。39 条 TS 基线 HTTP 路由、SSE 线格式与 socket.io 事件 1:1 冻结契约（后新增 `/api/auth/me`、`POST /api/chat/threads`、`POST /api/chat/upload`，现 42 条，均带 pytest 契约钉死），行为由 pytest 契约套件钉死。
...
| **服务端网关 (`services/gateway-py`)**   | Next.js API Routes 充当轻量接口，领域服务与控制器耦合  | **FastAPI 工业级 API 网关**：42 条契约路由（crud/admin/chat/spi 四路由组 + `/api/auth/me`、`POST /api/chat/threads`、`POST /api/chat/upload`），Pydantic 强类型 DTO 校验，租户+IP 滑动窗口限流，开放商户 SPI 协议，python-socketio 实时坐席协同通道。                                      |
...
│  - 42 条契约路由(39 冻结 + auth/me、chat/threads、chat/upload): tenants/skills/approvals/RAG/billing │
...
**契约冻结承诺**：39 条 TS 基线 HTTP 路由、SSE 事件线格式与 socket.io 事件名 1:1 冻结；冻结集合之外的新增路由（`/api/auth/me`、`POST /api/chat/threads`、`POST /api/chat/upload`，现合计 42 条）必须同批补契约测试。`services/gateway-py/tests/` 下的 pytest 契约套件（95 个用例，2026-09-08，含 HTTP/SSE/socket.io/AST 沙箱/图片上传）是唯一真实来源，任何契约改动必须同步更新测试与前端 `packages/types`。
...
| **FastAPI 核心后端网关 (`services/gateway-py`)** | [http://localhost:4000](http://localhost:4000) | 统一 API 网关、42 条契约路由、socket.io 协同、商户 SPI   |
...
```bash
# 1. 运行网关契约套件 (42 条路由 + SSE/socket.io 线格式, 密封容器)
bun run test:eval
