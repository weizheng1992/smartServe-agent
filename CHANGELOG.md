...
- **聊天气泡 GFM 表格不渲染**:客服 bot 产 GFM 管道表,但四个聊天面(merchant 悬浮助手 / web ChatArea / merchant-admin 直播台 / admin 深溯抽屉)全部 `whitespace-pre-wrap` 纯文本渲染,用户看到竖线原文 —— 平台级缺口(全仓零 markdown 依赖)。修复:packages/ui 新增零依赖 `MessageText` 原子(守 §5 禁重型库),手写最小 GFM 子集:表头+分隔行+数据行升级语义 `<table>`(分隔行定列对齐),缺分隔行/列数不符/行首竖线不成表一律诚实降级纯文本;四面统一换用,气泡样式不动。红环以事故载荷钉死(直通版先红:输出即竖线原文)。
- **咨询直答历史投毒(续编幻觉)**:thread 历史里躺着挂错租户时代的旧幻觉答案(「帐篷A/B/C款 ¥399/699/899」),次日同题再问走 consult 直答(rag_direct),`[RECENT CONVERSATION]` 装配无信任边界 —— RAG 明明已给真货(极光帐篷 ¥1299/¥1499),答案却把毒行逐字缝进新表。差分重放实锤(产线 limit=2 逐字重建事故 prompt):毒行在场 5/10 逐字复刻,毒行换诚实答案 0/10。修复:`answer_consult_from_rag` prompt 历史块显式降级「UNVERIFIED MEMORY, NOT A FACT SOURCE」,商品名/型号/价格只准从摘录逐字引用;「要对比多款而摘录不足」时只出摘录内条目,严禁填表式补行。修复后同环 0/10。回归:`test_consult_prompt_grounding.py` 钉 prompt 信任边界契约。

### 📝 Notes

- 症状②环层:整管线回放(run_agent)8/8 不红,prompt 是 (input, rag_documents, history, brand) 纯函数 → 差分重放直接打 LLM 才红 —— 非确定性 LLM 缺陷的环要建在最内层纯函数上。
- 已知残留(非本案机制,待观察):「几个X…对比」预设缺口在 RAG 素材较多(limit=5 形态)时仍可能诱发填表式编造;根治方向是商品对比问句路由到商品检索工具而非 RAG 直答。
