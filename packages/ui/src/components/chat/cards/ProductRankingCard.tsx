...
export const ProductRankingCard: React.FC<ProductRankingCardProps> = ({
  data,
  onSelectProduct,
}) => {
  ...
  return (
    <Card className="bg-slate-900/90 border-slate-800 text-slate-100 shadow-xl overflow-hidden max-w-lg">
      <CardHeader className="bg-slate-850/70 p-3.5 border-b border-slate-800 flex flex-row items-center justify-between">
        ...
        <Badge
          variant="outline"
          className="border-indigo-500/30 text-indigo-300 bg-indigo-950/20 text-[10px]"
        >
      ...
      <CardContent className="p-3 space-y-2 text-xs">
        {summary && (
          <p className="text-[11px] text-slate-400 pb-1 border-b border-slate-800/60">
            {summary}
          </p>
        )}
              ...
              >
                <div className="flex items-center space-x-2.5 min-w-0">
                  ...
                  <div className="min-w-0">
                    <h4 className="font-medium text-slate-200 truncate text-xs">
                      {p.name}
                    </h4>
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-[10px] text-slate-400">
                    毛利 ¥{p.grossProfit.toLocaleString()}
                  </div>
                </div>
              </div>
            );
