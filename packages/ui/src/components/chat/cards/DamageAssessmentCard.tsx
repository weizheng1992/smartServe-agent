...
import { AlertOctagon, AlertTriangle, Camera, CheckCircle2 } from '../../icons';

export interface DamageAssessmentCardProps {
  data: DamageAssessmentData;
}

export const DamageAssessmentCard: React.FC<DamageAssessmentCardProps> = ({ data }) => {
    ...
    <div className="my-2 max-w-md overflow-hidden rounded-xl border border-indigo-500/30 bg-gradient-to-b from-indigo-950/40 to-slate-900/90 p-4 text-slate-100 shadow-xl backdrop-blur-md">
      ...
      <div className="flex items-center justify-between border-b border-indigo-500/20 pb-3">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="text-xs font-semibold text-indigo-300">AI 视觉成色与定责智能评定</div>
            <div className="text-xs font-medium text-slate-400">置信度: {(data.confidence * 100).toFixed(0)}%</div>
          </div>
        </div>
        <span
        ...
        >
          {isSevere ? <AlertOctagon className="h-3 w-3" /> : <AlertTriangle className="h-3 w-3" />}
      ...
      <div className="my-3 space-y-2.5 text-xs">
        {data.imageUrl && (
          <div className="relative overflow-hidden rounded-lg border border-slate-700 bg-slate-950">
            <img src={data.imageUrl} alt="Uploaded Item" className="max-h-48 w-full object-cover" />
          </div>
        )}
