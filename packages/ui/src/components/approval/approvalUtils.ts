...
export function getApprovalCategory(actionType?: string | null): ApprovalCategory {
...
function resolveHumanEscalationCause(context: ApprovalContextData): string {
  ...
  if (src.includes('sentiment') || /(态度|生气|投诉|太差|听不懂|垃圾)/i.test(input)) {
    return '【情绪风控拦截】检测到用户负面情绪波动或强烈不满，系统主动挂起并升级至资深客服';
  }
  if (src.includes('circuit') || src.includes('retry') || /(重试|失败|一直报错)/i.test(input)) {
...
> = {
  refund: (ctx, raw) => ({
    ...
    ruleDescription: 'SOP 财务准则：单笔退款高于放行阈值或订单处于已发货/结算期，必须由人工管理员二次授权。',
  ...
  address: (ctx, raw) => ({
    ...
    triggerCause: ctx.reason || '订单已处于分拣/出库关键节点，修改跨省市地址存在物流窜货与冒领风险',
    ruleDescription: 'SOP 履约准则：出库后改派地址需人工核验运费差价并向承运物流商发起物理拦截。',
  ...
  human: (ctx, raw) => ({
    ...
    ruleDescription: 'SOP 体验准则：当检测到强诉求或情绪升级时，AI 助理立即挂起，交由坐席专家一对一处理。',
  ...
  generic: (ctx, raw) => ({
    ...
    triggerCause: ctx.reason || '调起外部未配置免密放行策略的操作，触发系统安全复核',
    ruleDescription: 'SOP 权限准则：涉及物理数据库变更或第三方 API 调用的高危操作需人工复核。',
...
export function diagnoseApprovalTrigger(approval: Approval): TriggerDiagnosis {
  ...
  const strategy = DIAGNOSIS_STRATEGIES[context.category] || DIAGNOSIS_STRATEGIES.generic;
