...
import { type TriggerDiagnosis, diagnoseApprovalTrigger, getApprovalContextData } from './approvalUtils';
  ...
  useEffect(() => {
    if (initialDetail) {
      ...
      if (approval.threadId) {
        setLoading(true);
        const fetchMessages = fetch(`/api/chat/messages?threadId=${encodeURIComponent(approval.threadId)}`)
          .then((res) => res.json())
          .catch(() => ({ success: false }));
        ...
        if (approval.businessId) orderParams.set('businessId', approval.businessId);
        ...
        Promise.all([fetchMessages, fetchPreferences, fetchOrders])
          .then(([msgData, prefData, ordData]) => {
            setDetail((prev) => {
              ...
              if (prefData.success && Array.isArray(prefData.preferences) && prefData.preferences.length > 0) {
                if (nextDetail.user) {
                  nextDetail.user.preferences = prefData.preferences.map((p: any) => ({
                    id: p.id,
                    fact: p.fact,
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex items-center justify-between">
                ...
                <div className="text-right text-xs text-slate-400 font-mono space-y-1">
                  <div>
                    历史订单数: <strong className="text-slate-200">{detail?.orders?.length || 0}</strong> 笔
                  </div>
                  <div>
                    提取画像数: <strong className="text-slate-200">{detail?.user?.preferences?.length || 0}</strong> 条
                  </div>
                </div>
              </div>
                    ...
                    >
                      <CardContent className="p-4 space-y-3.5">
                        ...
                        <div className="flex items-center justify-between pb-2 border-b border-slate-800/80">
                          <div className="flex items-center gap-2">
                            <span className="font-mono text-xs font-bold text-slate-100">订单号: {ord.orderId}</span>
                            {isTarget && (
                        ...
                        <div className="bg-slate-950/70 border border-slate-850 p-2.5 rounded-lg space-y-1.5 text-xs font-mono">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-1.5 text-slate-300">
                              ...
                              <span className="font-bold text-slate-200">{ord.recipientName || '收货人'}</span>
                              <span className="text-slate-400">({ord.phone || '138****0000'})</span>
                            </div>
                            {ord.addressId && (
                        ...
                        <div className="flex justify-between text-[11px] font-mono text-slate-500 pt-1 border-t border-slate-850">
                          <span>
                            承运物流: {ord.carrier || '顺丰速运'} ({ord.trackingNumber || '暂无单号'})
                          </span>
                          <span>
