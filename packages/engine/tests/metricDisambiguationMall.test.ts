...
import { METRIC_SEMANTIC_REGISTRY, MetricSemanticResolver, OrderDomainService } from 'tools';
import { CardSynthesizer } from '../src/cards/cardSynthesizer';
import { PRODUCT_RANKING_METRIC_SLOT, SlotDisambiguationEngine } from '../src/disambiguation/slotDisambiguationEngine';
...
describe('🏪 Mall Semantic Metric Registry & Disambiguation Suite', () => {
  ...
  it('MetricSemanticResolver: 模糊自然语言解析与冲突组消歧检测', () => {
    // 1. 模糊问句：命中冲突组并提示消歧
    const res1 = MetricSemanticResolver.resolve('帮我查一下我负责商品里面卖得最好的几个');
    ...
    // 4. 滞销库存预警问句
    const res4 = MetricSemanticResolver.resolve('仓库里积压最多、滞销的是哪几款');
  ...
  it('SlotDisambiguationEngine: 声明式槽位解析与动态快捷胶囊生成', () => {
    const res = SlotDisambiguationEngine.resolveSlot('查查卖得最好的', PRODUCT_RANKING_METRIC_SLOT);

    expect(res.resolvedValue).toBe('gmv');
    expect(res.quickReplies).toBeDefined();
    expect(res.quickReplies?.options.length).toBe(3);
    expect(res.quickReplies?.options.some((o) => o.label.includes('出货销量'))).toBe(true);
    expect(res.quickReplies?.options.some((o) => o.label.includes('净毛利润'))).toBe(true);
  });
  ...
  it('OrderDomainService.queryProductRanking: 物理 SQL 动态聚合与多维排行准确度', async () => {
    ...
    // 顶级跑鞋 / 飞马跑鞋 流水最高
    expect(gmvProducts[0].totalGmv).toBeGreaterThanOrEqual(gmvProducts[1].totalGmv);
  ...
  it('CardSynthesizer: 自动合成 product_ranking 富卡片与消歧胶囊', () => {
    ...
    expect(quickReplies?.data.options.some((o) => o.label.includes('出货销量'))).toBe(true);
    expect(quickReplies?.data.options.some((o) => o.label.includes('净毛利润'))).toBe(true);
  });
});
