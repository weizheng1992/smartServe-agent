...
import { approvalOutboxEvents, db, pendingApprovals as dbPendingApprovals, getDrizzle } from 'db';
...
describe('📮 Transactional Outbox & Deterministic Resumption Suite (TDD)', () => {
  ...
  it('💥 故障注入与对账补偿：派发阶段网络抛错崩溃，对账 Worker 能够自动捕获并成功恢复任务', async () => {
    ...
    (WorkflowOrchestrator as any).dispatchJob = async (opts: any) => {
      if (faultInjected) {
        throw new Error('Network timeout during Temporal/simulator dispatch (Simulated Crash)');
      }
    ...
    try {
      ...
      const summaryIdempotent = await ApprovalOutboxWorker.processPendingEvents(0);
