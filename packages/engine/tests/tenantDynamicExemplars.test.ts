...
describe('租户级动态 Exemplar 样本路由测试 (Tenant-Isolated Dynamic Exemplar Routing)', () => {
  ...
  test('应该能按租户隔离精确召回样本 (Nike vs Adidas 物理隔离)', async () => {
    // 为 Adidas 添加一条不同样本
    await ExemplarService.addExemplar(tenantAdidas, 'boost_sole_maintenance', '我的椰子鞋底发黄了怎么清洗保养');

    // 查询 Nike
    const nikeResults = await ExemplarService.searchRelevantExemplars(tenantNike, '防伪码验证球鞋真伪');
    ...
    const adidasResults = await ExemplarService.searchRelevantExemplars(tenantAdidas, '鞋底发黄清洗保养');
    ...
    const nikeCrossResults = await ExemplarService.searchRelevantExemplars(tenantNike, '椰子鞋底保养');
    const hasAdidasIntent = nikeCrossResults.some((r) => r.intentName === 'boost_sole_maintenance');
  ...
  test('当样本不存在时应平滑返回空数组降级', async () => {
    const emptyResults = await ExemplarService.searchRelevantExemplars('non_existing_tenant', '测试语句');
