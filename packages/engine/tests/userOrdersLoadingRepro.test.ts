...
describe("Diagnose '我的订单' loading and card rendering issue", () => {
  test("runAgent with '我的订单' should list orders and synthesize order cards", async () => {
    ...
    const result = await runAgent(threadId, userId, '我的订单', `job_${Date.now()}`);
    ...
    const listSubtask = result.taskPlan?.subtasks?.find(
      (st) => st.id === 'step_fast_list_orders' || st.description.toLowerCase().includes('listuserorders'),
    );
    ...
    console.log('Synthesized Cards:', JSON.stringify(synthesizedCards, null, 2));
    expect(synthesizedCards.length).toBeGreaterThan(0);
    expect(synthesizedCards.some((c) => c.type === 'order_card' || c.type === 'order_picker')).toBe(true);
  }, 30000);
});
