...
describe('Executor Node Unit Tests', () => {
  test('returns globalTransitionsCount 1 when no subtask exists at currentIndex', async () => {
    ...
    const result = await executorNode(state as typeof AgentStateAnnotation.State);
  ...
  test('Fast-Path human escalation creates pending approval ticket and suspends', async () => {
        ...
        subtasks: [
          {
            ...
            description: 'Trigger human escalation and create pending approval ticket',
            status: 'pending',
          },
        ],
        currentStepIndex: 0,
    ...
    expect(result.taskPlan.subtasks[0].result?.actionType).toBe('human_escalation');
  ...
  test('Fast-Path getOrderStatus executes physical tool without calling LLM', async () => {
    ...
    expect(result.taskPlan).toBeDefined();
    expect(result.taskPlan.subtasks[0].status).toBe('completed');
    expect(result.taskPlan.subtasks[0].result?.toolExecuted).toBe('getOrderStatus');
    expect(result.taskPlan.subtasks[0].result?.output?.orderId).toBe('ORD-98712');
  });
  ...
  test('Fast-Path listUserOrders executes tool directly for order listing', async () => {
    ...
    expect(result.taskPlan).toBeDefined();
    expect(result.taskPlan.subtasks[0].status).toBe('completed');
    expect(result.taskPlan.subtasks[0].result?.toolExecuted).toBe('listUserOrders');
  });
});
