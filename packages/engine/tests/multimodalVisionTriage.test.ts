...
describe('📷 Multimodal Vision & Rich Cards Suite', () => {
  it('VisionAnalyzerService: 自动识别破损照片并生成定责评估', async () => {
    ...
    expect(res.damageAssessment?.damageLevel).toBeOneOf(['minor', 'severe', 'negligible']);
    expect(res.damageAssessment?.imageUrl).toBe('https://cdn.store.com/uploads/broken_shoes.jpg');
  });
