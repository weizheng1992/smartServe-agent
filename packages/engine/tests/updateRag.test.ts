...
import { deleteChunksBySource, replaceKnowledgeFile, upsertDocumentChunk } from '../src/rag/updateRag';
...
describe('RAG Update & Replacement Pipeline Tests', () => {
  ...
  test('Should replace full knowledge file and delete old chunks', async () => {
    const filePath = path.resolve(__dirname, '../../../docs/knowledge/nike_store_and_products.md');
  ...
  test('Should safely delete chunks by sourceUrl', async () => {
    const deletedCount = await deleteChunksBySource('nike', 'test_update_file.md');
