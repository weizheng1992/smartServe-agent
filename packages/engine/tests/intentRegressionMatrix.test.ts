...
describe('全场景全方位电商客服意图与业务闭环测试矩阵 (Comprehensive Matrix)', () => {
  describe('1. 槽位提取与意图识别全覆盖 (Slot Extraction & Disambiguation)', () => {
    test('【查订单/泛查单】不应索要订单号，直接走 listUserOrders', () => {
      const inputs = ['我的订单', '看看我买了啥', '我有哪些订单', '订单列表', '历史购买记录', '查下我买的东西'];
    ...
    test('【修改地址】全缺槽位 / 部分缺失 / 完整指令三级状态机', () => {
      ...
      // 2. 缺新地址
      const missingAddr = SlotExtractor.extract('把订单 ORD-998877 改一下收货地址');
    ...
    test('【退货与退款】有单号直接流转，无单号精准追问', () => {
      ...
      // 有单号
      const hasId = SlotExtractor.extract('订单 ORD-889901 我不想要了，帮我退款');
    ...
    test('【取消订单】待发货取消意图与单号解析', () => {
      const cancelWithId = SlotExtractor.extract('帮我取消订单 ORD-ECOM-889901');
  ...
  describe('2. 多意图与复合指令 DAG 规划 (Multi-Intent Composite Planning)', () => {
    test('查物流 + 申请退款 双意图应生成对应两个子任务', async () => {
      ...
      const toolDescriptions = planRes.taskPlan?.subtasks?.map((s) => s.description.toLowerCase()) || [];
      expect(toolDescriptions.some((d) => d.includes('getorderstatus') || d.includes('status'))).toBe(true);
      expect(toolDescriptions.some((d) => d.includes('processrefund') || d.includes('refund'))).toBe(true);
    });
  });
  ...
  describe('3. 多租户品牌隔离与自识别问单 (Multi-Tenant Isolation Matrix)', () => {
    test("在 Adidas 店铺下提问'我的订单'应正常响应 Adidas 品牌且输出订单卡片", async () => {
      ...
      const result = await runAgent(threadId, userId, '我的订单', `job_${Date.now()}`, undefined, businessId);
      ...
      expect(cards.some((c) => c.type === 'order_card' || c.type === 'order_picker')).toBe(true);
    }, 300000);
