...
describe('Human Support Operator Takeover & IM Chat Full E2E Lifecycle Test', () => {
  test('Full lifecycle: User Escalation -> Pending Approval Ticket -> Human Reply -> Resume Agent', async () => {
    ...
    const userEscalationMessage = '请问有人吗？我要转接人工客服处理我的换货请求！';

    console.log('[E2E Test Step 1] Step 1: User requests human support escalation...');

    // 1. Run agent with user escalation message
    const initialResult = await runAgent(threadId, userId, userEscalationMessage);

    expect(initialResult).toBeDefined();
    expect(initialResult.output).toBeDefined();
    expect(initialResult.output).toContain('人工客服');

    console.log('[E2E Test Step 1 Success] Output:', initialResult.output.substring(0, 100));

    // 2. Verify physical database state for pending_approvals record
    console.log('[E2E Test Step 2] Step 2: Verifying database pending_approvals record...');
    ...
    console.log('[E2E Test Step 2 Success] Found pending ticket ID:', pendingTicket.id);

    // 3. Verify conversation history fetching for Admin IM Chat Modal
    console.log('[E2E Test Step 3] Step 3: Fetching thread conversation history for Admin IM Modal...');
    ...
    const userMsg = threadMessages.find((m: { role: string }) => m.role === 'user');
    expect(userMsg).toBeDefined();
    expect(userMsg.content).toBe(userEscalationMessage);

    console.log('[E2E Test Step 3 Success] Thread history contains', threadMessages.length, 'messages.');

    // 4. Simulate Human Operator sending reply via Admin Approval API handler
    console.log('[E2E Test Step 4] Step 4: Simulating Human Support Operator reply & ticket resolution...');
    const operatorReplyText = '您好！我是人工高级客服主管。我已核实您的情况，将为您进行退换货绿色通道安排。';
    ...
    expect((updatedTickets[0].actionPayload as any).humanReply).toBe(operatorReplyText);

    console.log('[E2E Test Step 4 Success] Approval ticket status updated to resolved_by_human.');

    // 6. Resume Agent execution with System prompt injection
    console.log('[E2E Test Step 5] Step 5: Resuming agent workflow with human reply context...');
    ...
    console.log('[E2E Test Step 5 Success] Resumed Output:', resumeResult.output.substring(0, 100));
    ...
    console.log('[E2E Test Complete] All 7 steps in the Human Support E2E lifecycle test passed!');
  }, 120000);
});
