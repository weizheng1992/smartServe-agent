...
describe('Merchant User Orders Query Bug Reproduction', () => {
  ...
  it('should resolve user orders for 张伟 (CUST-8801) in aurora merchant', async () => {
    ...
    console.log('[Test Repro Cards]:', JSON.stringify(finalState?.cards, null, 2));
    console.log('[Test Repro Subtasks]:', JSON.stringify(finalState?.taskPlan?.subtasks, null, 2));

    // 断言不应该报找不到用户上下文或未能获取登录状态
    expect(finalState?.taskPlan?.subtasks?.[0]?.result?.output?.error).toBeUndefined();
    ...
    expect(finalState?.cards?.some((c: any) => c.type === 'order_card' || c.type === 'order_picker')).toBe(true);
  }, 20000);
});
