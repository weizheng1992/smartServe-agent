...
export class AgentMemoryEngine {
  ...
  constructor(threadId: string, userId: string, maxTurns = 10, businessId?: string) {
    this.threadId = threadId;
    this.userId = userId;
    this.businessId = businessId;

    this.short = new ShortMemory(threadId, maxTurns);
    this.long = new LongMemory(userId, businessId);
    this.task = new TaskMemory(threadId);
    this.episodic = new EpisodicMemory(userId, businessId);
  }
