...
export class ContextAssemblyPipeline {
   ...
   * 格式化短期对话历史
   */
  public static formatShortHistory(
   ...
   * 格式化 RAG 知识库检索切片
   */
  public static formatRagDocs(
    docs?: Array<{ title?: string; content: string; score?: number }>,
    maxCount = 3,
  ): string {
   ...
   * 格式化情景记忆事件
   */
  public static formatEpisodicEvents(
  ...
  public static async assemble(
  ...
  ): Promise<AssembledContextBundle> {
    const {
      threadId,
      userId,
      query,
    ...
    const conversationHistoryText = this.formatShortHistory(
      activeShortMessages || [],
      maxShortMessages,
    );
    ...
    if (userPersonaText && userPersonaText !== "无记录画像") {
      sections.push(
        `【客户长期画像与偏好 (User Persona Facts)】\n${userPersonaText}`,
      );
    }

    if (episodicEventsText && episodicEventsText !== "无历史情景") {
      sections.push(
        `【历史相似交互情景 (Episodic Context)】\n${episodicEventsText}`,
      );
    }

    if (ragKnowledgeText && ragKnowledgeText !== "无匹配的知识库内容") {
      sections.push(
        `【知识库参考文档 (Retrieved Knowledge Base)】\n${ragKnowledgeText}`,
      );
    }

    sections.push(
      `【近期对话上下文 (Recent Conversation History)】\n${conversationHistoryText}`,
    );
