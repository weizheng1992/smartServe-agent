...
export const currentPlanQuery = defineQuery<any>('currentPlan');
export const chatHistoryQuery = defineQuery<{ role: string; content: string }[]>('chatHistory');

export async function agentWorkflow(threadId: string, userId: string, inputMessage: string): Promise<WorkflowResult> {
  // 本地 Workflow 状态变量
  let currentStatus = '初始化工作流编排器...';
  const chatHistory: { role: string; content: string }[] = [{ role: 'user', content: inputMessage }];
  ...
  if (triageState.intents && triageState.intents.length > 0) {
    const intentsStr = triageState.intents.map((p: any) => `${p.intent} (${p.confidence})`).join(', ');
    currentStatus = `[Triage 完成] 识别出核心意图: ${intentsStr}`;
  } else {
    currentStatus = '[Triage 完成] 未识别出核心意图，准备直接交由 Finish 节点处理。';
  }
  ...
  if (!triageState.intents || triageState.intents.length === 0 || isOnlyGeneralQuery) {
    ...
    chatHistory.push({ role: 'assistant', content: finishedState.output || '' });
  ...
  currentStatus = '[Planner] 正在根据识别到的意图，实时物理规划 DAG 任务执行拓扑图...';
  ...
  while (loopCount < maxLoops) {
    ...
    if (executedSubtask?.result) {
      ...
      if (res.toolExecuted === 'getOrderStatus') {
        ...
        currentStatus = '[物理工具 takeScreenshot 调用完成] 成功渲染后台网页并捕获看板快照图片！';
      } else {
  ...
  currentStatus = '[Finish] 正在整合全部物理工具调用结果，通过大模型组织人性化中文最终回复...';
