...
function scrubSensitiveInfo(text: string): string {
  if (!text) return text;
  return text
    .replace(/(?:\+?86)?1[3-9]\d{9}/g, (m) => `${m.substring(0, 3)}****${m.substring(7)}`) // 手机号
    .replace(/\b\d{6}(18|19|20)?\d{2}(0[1-9]|1[0-2])(0[1-9]|[12]\d|3[01])\d{3}[\dXx]\b/g, '[ID_CARD_REDACTED]') // 身份证
 ...
 */
function buildHeuristicDamageAssessment(combinedQuery: string, primaryUrl: string): DamageAssessmentData | undefined {
  const isDamage = /破损|坏了|碎了|裂开|漏液|划痕|撕裂|瑕疵|damage|broken|crack|stain|defect/i.test(combinedQuery);
  ...
  return {
    ...
    summary: isSevere ? '用户上传了商品严重破损/碎裂照片' : '用户上传了商品瑕疵/局部破损凭证照片',
    confidence: 0.88,
...
export class VisionAnalyzerService {
   ...
   */
  static async analyzeImages(imageUrls: string[], userPrompt = '', jobId?: string): Promise<VisionAnalysisResult> {
    ...
    const orderMatch = rawText.match(/\bORD-[A-Za-z0-9_-]+\b/i)?.[0]?.toUpperCase();
    const trackingMatch = rawText.match(/\b(SF|YTO|ZTO|EMS|TRACK)[\d\w]{8,14}\b/i)?.[0]?.toUpperCase();
...
Return ONLY valid raw JSON without markdown markers.`;
      ...
      const response = await Promise.race([llm.invoke([humanMsg]), timeoutPromise]);
      ...
      const damageAssessment: DamageAssessmentData | undefined = parsed.damageAssessment
        ? {
            damageLevel: parsed.damageAssessment.damageLevel || 'minor',
            summary: scrubSensitiveInfo(parsed.damageAssessment.summary || '商品外观检测'),
            confidence: parsed.damageAssessment.confidence || 0.85,
            suggestedAction: parsed.damageAssessment.suggestedAction || 'human_review',
            imageUrl: primaryUrl,
          }
      ...
      return {
        ...
        extractedTrackingNumber: parsed.extractedTrackingNumber || trackingMatch,
        ocrText: scrubSensitiveInfo(parsed.ocrText || ''),
        damageAssessment,
      };
    } catch (visionErr) {
      console.warn('[VisionAnalyzerService] Multimodal LLM inspection fallback to heuristics:', visionErr);

      // 降级兜底：基于统一启发式规则返回结构化视觉分析
      const damageAssessment = buildHeuristicDamageAssessment(combinedQuery, primaryUrl);
