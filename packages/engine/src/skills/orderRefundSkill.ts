...
import type { RichCardBlock, SkillExecutionContext, SkillExecutionResult, SkillMetadata } from 'types';
...
export class OrderRefundSkill extends BaseSkill {
  ...
  public async execute(context: SkillExecutionContext): Promise<SkillExecutionResult> {
    ...
    const reason = (context.slots?.reason as string) || (context.slots?.returnReason as string) || '客户申请退款';
    ...
    const requestedAmount =
      Number.parseFloat(String(context.slots?.refundAmount || order.totalAmount)) || totalAmountNum;

    // Step 2: HITL 安全门禁检查 (超过 50 元阈值且未直接放行时挂起审批)
    const threshold = this.metadata.approvalThresholdAmount || 50;
