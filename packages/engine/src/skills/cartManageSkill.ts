...
export class CartManageSkill extends BaseSkill {
  ...
  public canHandle(context: SkillExecutionContext): boolean {
    ...
    return /(?:加购物车|加入购物车|放进购物车|加购|购物车|结算|买第|件加入|放入购物车)/i.test(
      input,
    );
  }
  ...
  public async execute(
    ...
    const isViewOnly =
      /(?:查看购物车|看下购物车|购物车里|购物车有什么|多少钱|算下总价|结算|去买单)/i.test(
        input,
      ) && !/(?:加|买|放)/i.test(input);
    ...
    if (isViewOnly) {
      ...
      const card: RichCardBlock = {
        ...
        data: {
          ...
          payableAmount: cartData.payableAmount,
        },
      };
    ...
    const updateMatch = input.match(
      /(?:改成|修改为|数量设为|变成|改为)\s*(\d+)\s*件?/,
    );
    if (updateMatch && updateMatch[1]) {
      const newQty = Number(updateMatch[1]);
      const targetSku =
        existingCart.lastModifiedItemId || "sku_nike_aj1_blk_425";
    ...
    if (!targetSkuId) {
      // 默认兜底添加热销款
      targetSkuId =
        guideContext.candidateProductIds?.[0] || "prod_nike_air_pegasus_41";
