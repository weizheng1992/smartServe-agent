...
export interface ScoredRAGDocument extends RagDocument {
  id: string;
  businessId: string;
  chunkText: string;
...
function computeBM25(
  ...
  for (const token of queryTokens) {
    ...
    const idfValue = Math.log(
      Math.max(0.0001, (N - n_q + 0.5) / (n_q + 0.5) + 1),
    );
...
export class ContextualRAG {
  ...
  private async ensureSeedData(): Promise<void> {
    ...
    try {
      ...
      if (existing.length === 0) {
        console.log(
          "[RAG] 📦 检测到 RAG 知识库为空，启动自动高保真 Contextual RAG 数据自愈注入...",
        );
        ...
        const seedDocs = [
          ...
          },
          {
            ...
            category: "size_chart",
            contextualSummary:
              "这段切片详细规定了 Adidas 衣服欧版偏宽松落肩的设计特征及建议买小一码的尺码指南。",
          },
          {
            businessId: "ecommerce",
            chunkText:
            ...
            category: "size_chart",
            contextualSummary:
              "这段切片规定了电商主站针织衫等标准国标尺码对照，以及考虑纯棉缩水率后的微胖大一码推荐。",
          },
        ];
        ...
        console.log(
          "[RAG] ✅ Contextual RAG 演示数据自动注入成功！已永久落盘 Postgres 物理表。",
        );
      }
    } catch (err) {
      console.warn(
        "[RAG] Self-healing seed failed (possibly due to offline/mocked DB):",
        err,
      );
    }
  }
  ...
  async searchRelevantDocs(
    ...
    if (!queryEmbedding || queryEmbedding.length === 0) {
      ...
      try {
        ...
        console.error(
          "[RAG] Failed to generate embedding for search query:",
          err,
        );
        return [];
      }
    ...
    if (!drizzle) {
      console.log(
        "[RAG] PostgreSQL 处于离线状态，无缝启用 Local Fake RAG 高保真模拟检索...",
      );
    ...
    try {
      ...
      console.log(
        `[RAG] PostgreSQL 物理混合检索检索完成，经 BM25 + RRF 重排筛选出 ${sorted.length} 个相关切片。`,
      );
      return sorted;
    } catch (dbErr) {
      console.warn(
        "[RAG] PostgreSQL query failed, falling back to Local Fake RAG:",
        dbErr,
      );
      return this.searchLocalFakeDocs(query);
    }
  ...
  private searchLocalFakeDocs(query: string): ScoredRAGDocument[] {
    ...
    const results = filtered.map((d) => {
      ...
      const hybridScore =
        simulatedVectorSimilarity * 0.8 + normalizedBM25 * 0.2;
