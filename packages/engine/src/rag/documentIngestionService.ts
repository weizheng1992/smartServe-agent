...
export function chunkDocumentText(params: ChunkDocumentParams): ChunkResult[] {
  const { text, targetChunkSize = 600, overlap = 100, headerPath = '通用说明' } = params;
...
export function buildContextualSummaryPrompt(params: BuildContextualPromptParams): string {
  const { fullDocumentText, documentOutline, sectionContext, chunkText, businessId, brandName = businessId } = params;
