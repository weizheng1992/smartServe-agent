...
export class CardSynthesizer {
  /**
   * 生成流式骨架卡片 (Skeleton Cards for Streaming Hydration)
   */
  static synthesizeSkeletonCards(options: SynthesizeCardsOptions): RichCardBlock[] {
    const skeletons: RichCardBlock[] = [];
    const { intents, taskPlan } = options;

    if (taskPlan?.subtasks && taskPlan.subtasks.length > 0) {
      for (const st of taskPlan.subtasks) {
        if (st.id?.includes('order') || st.id?.includes('status')) {
          skeletons.push({
            id: `skel_order_${st.id}`,
            type: 'order_card',
        ...
        } else if (st.id?.includes('refund') || st.id?.includes('step')) {
          skeletons.push({
            id: `skel_step_${st.id}`,
            type: 'step_progress',
            hydrationState: 'skeleton',
      ...
      for (const it of intents) {
        if (it.intent === 'order_status') {
          skeletons.push({
            id: `skel_intent_order_${it.intent}`,
            type: 'order_card',
            hydrationState: 'skeleton',
            data: {
              orderId: 'ORD-LOADING...',
  ...
  static synthesizeCards(options: SynthesizeCardsOptions): RichCardBlock[] {
    ...
    if (taskPlan?.subtasks) {
      for (const st of taskPlan.subtasks) {
        const result = (st.result?.output || st.result) as Record<string, unknown> | undefined;
        if (!result || result.error) continue;

        const orderId = typeof result.orderId === 'string' ? result.orderId : undefined;
        const status = typeof result.status === 'string' ? result.status : undefined;
        const carrier = typeof result.carrier === 'string' ? result.carrier : undefined;
        const trackingNumber = typeof result.trackingNumber === 'string' ? result.trackingNumber : undefined;

        // 2. 识别并提取订单详情卡与物流轨迹卡 (来自于 getOrderStatus / listUserOrders)
        const ordersList: Array<Record<string, unknown>> = Array.isArray(result.orders)
          ? (result.orders as Array<Record<string, unknown>>)
          : orderId && (status || carrier || trackingNumber)
            ? [result]
            : [];

        const synthesizedOrderCards: OrderCardData[] = [];
        ...
        for (const ord of ordersList) {
          ...
          const ordTrackingNumber = (ord.trackingNumber as string) || (ord.tracking_number as string) || '暂无运单号';
          if (!ordId) continue;
          ...
          const orderCard: OrderCardData = {
            ...
            createdAt: (ord.createdAt as string) || (ord.created_at as string) || new Date().toISOString(),
            actions: [
              {
                label: '查看物流轨迹',
                action: 'track_order',
        ...
        if (isRefundTask && orderId) {
          ...
          const parsedRefundAmount =
            typeof rawRefundAmount === 'number'
              ...
              : typeof rawRefundAmount === 'string'
                ? Number.parseFloat(rawRefundAmount.replace(/[^0-9.]/g, '')) || 0
                : 0;
          ...
          const refundCard: RefundConfirmationData = {
            ...
            status: result.waitingForApproval ? 'pending_confirmation' : result.success ? 'approved' : 'submitted',
        ...
        // 5. 识别并合成业务流转步进卡 (StepProgressCard)
        const stepProgressData =
          (result.stepProgress as StepProgressCardData | undefined) ||
          (result.workflowSteps as StepProgressCardData | undefined);

        if (stepProgressData && Array.isArray(stepProgressData.steps)) {
          cards.push({
            type: 'step_progress',
            hydrationState: 'ready',
            data: {
              ticketId: stepProgressData.ticketId || (result.ticketId as string) || undefined,
              orderId: stepProgressData.orderId || orderId,
              title: stepProgressData.title || '业务售后流转追踪',
              currentStep: stepProgressData.currentStep ?? 0,
              totalSteps: stepProgressData.totalSteps || stepProgressData.steps.length,
              steps: stepProgressData.steps,
              settledSummary: stepProgressData.settledSummary,
            },
          });
        }

        // 6. 识别并合成多规格交互式商品卡 (InteractiveProductCard)
        const interactiveProduct =
          (result.interactiveProduct as InteractiveProductCardData | undefined) ||
          (result.productDetail as InteractiveProductCardData | undefined);

        if (interactiveProduct && Array.isArray(interactiveProduct.skus) && interactiveProduct.skus.length > 0) {
          cards.push({
            type: 'interactive_product',
            hydrationState: 'ready',
            data: {
              productId: interactiveProduct.productId || 'PROD-DEFAULT',
              title: interactiveProduct.title,
              subtitle: interactiveProduct.subtitle,
              imageUrl: interactiveProduct.imageUrl,
              basePrice: interactiveProduct.basePrice || interactiveProduct.skus[0]?.price || 0,
              skus: interactiveProduct.skus,
              selectedSkuId: interactiveProduct.selectedSkuId || interactiveProduct.skus[0]?.skuId,
              selectedQuantity: interactiveProduct.selectedQuantity || 1,
              actions: interactiveProduct.actions,
            },
          });
        }
      }
