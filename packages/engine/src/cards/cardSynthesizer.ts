...
export class CardSynthesizer {
   ...
   */
  static synthesizeSkeletonCards(
    options: SynthesizeCardsOptions,
  ): RichCardBlock[] {
    ...
    if (taskPlan?.subtasks) {
      for (const st of taskPlan.subtasks) {
        ...
        if (stepProgressData && Array.isArray(stepProgressData.steps)) {
          cards.push({
            ...
            data: {
              ...
              totalSteps:
                stepProgressData.totalSteps || stepProgressData.steps.length,
        ...
        const interactiveProduct =
          (result.interactiveProduct as
            InteractiveProductCardData | undefined) ||
          (result.productDetail as InteractiveProductCardData | undefined);

        if (
          interactiveProduct &&
          Array.isArray(interactiveProduct.skus) &&
          interactiveProduct.skus.length > 0
        ) {
          cards.push({
            ...
            data: {
              ...
              basePrice:
                interactiveProduct.basePrice ||
                interactiveProduct.skus[0]?.price ||
                0,
              skus: interactiveProduct.skus,
              selectedSkuId:
                interactiveProduct.selectedSkuId ||
                interactiveProduct.skus[0]?.skuId,
