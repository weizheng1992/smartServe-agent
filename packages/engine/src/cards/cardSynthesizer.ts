    ...
    if (taskPlan?.subtasks) {
      for (const st of taskPlan.subtasks) {
        ...
        for (const ord of ordersList) {
          ...
          const ordTrackingNumber = ord.trackingNumber || (ord.tracking_number as string) || '暂无运单号';
          if (!ordId) continue;
          ...
          const orderCard: OrderCardData = {
            ...
            createdAt: ord.createdAt || (ord.created_at as string) || new Date().toISOString(),
            actions: [
              {
                label: '查看物流轨迹',
                action: 'track_order',
          ...
          if (ordersList.length === 1 && (ordTrackingNumber || ordCarrier)) {
            ...
            const timelineCard: TrackingTimelineData = {
              ...
              estimatedDelivery:
                ord.estimatedDelivery || (ord.estimated_delivery as string) || '预计 1-3 个工作日内送达',
