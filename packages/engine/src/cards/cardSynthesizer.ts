  ...
  static synthesizeCards(options: SynthesizeCardsOptions): RichCardBlock[] {
    ...
    if (taskPlan?.subtasks) {
      for (const st of taskPlan.subtasks) {
        ...
        if (orderId && (status || carrier || trackingNumber)) {
          const rawAmount = result.totalAmount || result.amount;
          const parsedAmount =
          ...
          const orderCard: OrderCardData = {
            ...
            carrier: carrier || '标准快递',
            trackingNumber: trackingNumber || '暂无运单号',
          ...
          if (trackingNumber || carrier) {
            const rawTimeline = Array.isArray(result.timeline)
              ? (result.timeline as TrackingTimelineData['timeline'])
              : [
                  {
                    time: new Date().toISOString().replace('T', ' ').slice(0, 16),
                    location: `${carrier || '快递公司'} 派送中`,
                    description: `包裹正在运送中，状态：${status || '运输中'}`,
            ...
            const timelineCard: TrackingTimelineData = {
              trackingNumber: trackingNumber || '运单生成中',
              carrier: carrier || '快递承运商',
              currentStatus: status || '运输中',
              estimatedDelivery: (result.estimatedDelivery as string) || '预计 1-3 个工作日内送达',
