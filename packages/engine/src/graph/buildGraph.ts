...
export async function runAgent(
  ...
  // Initialize memories
  const shortMemory = new ShortMemory(threadId, 10, overrideBusinessId);
  ...
  if (isQuickGreeting(inputMessage)) {
    ...
    const greetingText = `您好！我是 ${brandName} 的智能客服助理。✨
...
请告诉我您需要处理的业务，我将真刀真枪为您调起系统底层工具为您搞定！`;
  ...
  if (jobId) {
    agentEventEmitter.emit(`${jobId}:result`, finalResult);
  }
