...
export function tryMatchExecutorFastPath(
  ...
  const isPureCommunicationStep =
    /^(present|ask|inform|explain|tell|show|display|向用户|询问|告知|说明|解释)/i.test(
      descLower,
    ...
    (descLower.includes("processrefund") ||
      ...
      (descLower.includes("refund") &&
        (descLower.includes("call") ||
          descLower.includes("execute") ||
          descLower.includes("initiate")))) &&
  ...
  if (
    ...
    let prefType = "other";
    if (
      inputLower.includes("码") ||
      inputLower.includes("尺码") ||
      inputLower.includes("size")
    ) {
      prefType = "size";
    } else if (
      inputLower.includes("色") ||
      inputLower.includes("颜色") ||
      inputLower.includes("color")
    ) {
      prefType = "color";
    } else if (
      inputLower.includes("牌") ||
      inputLower.includes("品牌") ||
