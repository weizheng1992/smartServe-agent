import { pendingApprovals as dbPendingApprovals, getDrizzle, db as physicalDb } from 'db';
...
export async function createPendingApprovalTicket({
  ...
  try {
    await physicalDb.createThread(threadId, userId || '83d67d4e-104c-4325-8aa7-10d4389fc725');
  } catch (tErr) {
    console.warn('[Executor Approvals] Thread ensure warning:', tErr);
  }

  let approvalId = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2, 15);
  ...
  if (existingApprovals.length > 0) {
    ...
    console.log(`[Executor Approvals] 🎯 Thread ${threadId} 已存在挂起中的人工工单 (${approvalId})，无需重复创建！`);
  } else {
