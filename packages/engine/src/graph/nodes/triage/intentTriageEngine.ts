...
请告诉我您需要处理的业务，我将直接为您调起系统底层工具为您搞定！`;
    ...
    try {
      ...
      // 判定 1: 复合意图直达
      if (scoreOrder >= 0.85 && scoreRefund >= 0.85 && Math.abs(scoreOrder - scoreRefund) < 0.15) {
      ...
      if (scoreOrder >= 0.88 && scoreOrder - scoreOos >= 0.08) {
        ...
        await this.logIntentToDB(threadId, input, intents, 'embedding', scoreOrder);
        return {
          intents,
          shortMemory: historyMsgs,
          globalTransitionsCount: -1,
      ...
      if (scoreRefund >= 0.88 && scoreRefund - scoreOos >= 0.08) {
        ...
        await this.logIntentToDB(threadId, input, intents, 'embedding', scoreRefund);
        return {
          intents,
          shortMemory: historyMsgs,
          globalTransitionsCount: -1,
