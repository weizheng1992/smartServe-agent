...
import { type AgentDomainRole, type DamageAssessmentData, getMerchantDisplayName } from 'types';
...
export function resolveDomainRole(intents: IntentResult[], input?: string): AgentDomainRole {
  ...
  if (
    ...
    /(?:推荐|买什么|挑一款|选一款|好看|款式|选鞋|选衣服|哪款好)/i.test(input || '')
  ) {
...
export class IntentTriageEngine {
   ...
   */
  static async logLowConfidenceToDB(threadId: string, inputText: string, candidates: unknown): Promise<void> {
    try {
      await db.execute(
        'INSERT INTO low_confidence_logs (thread_id, input_text, candidates, reviewed, created_at) VALUES ($1, $2, $3, false, $4)',
        [threadId, inputText, JSON.stringify(candidates), new Date().toISOString()],
      );
      console.log(`[Low Confidence Logging] ⚠️ Recorded low confidence query for human review -> thread: ${threadId}`);
    } catch (err) {
      console.warn('[Low Confidence Logging Exception] Bypassed log persistence:', err);
    }
  }
  ...
  static async handleImmediateBypass(
    ...
    const finalIntents = intents.length > 0 ? intents : [{ intent: 'general_query', confidence }];
  ...
  static async process(state: typeof AgentStateAnnotation.State) {
    ...
    try {
      ...
      if (!isOperationalAction && userMsgs.length >= 2 && assistantMsgs.length > 0) {
        ...
        const hasUnsanitizedTags = /\[(?:ECOMMERCE|BRAND|STORE|MERCHANT|SHOP|ADIDAS|NIKE)\]/i.test(
          lastAssistantMsg.content,
        );

        if ((isExactlySame || isSemanticallySame) && !isLastResponseFailed && !hasUnsanitizedTags) {
...
请告诉我您需要处理的业务，我将直接为您调起系统底层工具为您搞定！`;
    ...
    try {
      ...
      const matchedOrderId =
        input.match(/(?:[A-Za-z0-9]+-)*ORD-[A-Za-z0-9_-]+/i)?.[0] || visionAnalysis?.extractedOrderId;
    ...
    // =========================================================================
    // 🛡️ Step 3: 大模型 Deep Triage 结构化联合精判 (Structured Joint Triage)
    // =========================================================================
    console.log('[Triage Fallthrough to Step 3] Launching Structured Output deep triage classifier...');
    ...
    try {
      ...
      // 动态召回租户专属 Few-Shot 意图示例 (复用单次预计算的 inputEmbedding)
      let exemplarsPrompt = '';
      try {
        const matchedExemplars = await ExemplarService.searchRelevantExemplars(
          activeTenantId,
          input,
          state.inputEmbedding,
          3,
        );
        if (matchedExemplars.length > 0) {
          exemplarsPrompt = ExemplarService.formatExemplarsForPrompt(matchedExemplars);
          console.log(
            `[Triage Exemplars] 🎯 为商户 [${activeTenantId}] 动态装配 ${matchedExemplars.length} 条专属 Few-Shot 样本`,
          );
        }
      } catch (exErr) {
        console.warn('[Triage Exemplars Retrieval Exception]:', exErr);
      }

      const structuredRes = await StructuredClassifier.classify({
        input,
      ...
      const isOos = structuredRes.isOutOfScope || structuredRes.intents.some((p) => p.intent === 'out_of_scope');
      ...
      const parsed: IntentResult[] = structuredRes.intents.map((item, idx) => {
        const primaryOrderId = item.entities?.orderId || fallbackOrderId || undefined;
        return {
          intent: item.intent,
          confidence: item.confidence || 0.9,
          type: (item.type as 'primary' | 'secondary') || (idx === 0 ? 'primary' : 'secondary'),
          entities: {
            ...(item.entities || {}),
      ...
      const primaryOrderId = parsed.find((p) => p.entities?.orderId)?.entities?.orderId;
      if (primaryOrderId) {
        state.orderContext = {
          ...(state.orderContext || {}),
          targetOrderId: String(primaryOrderId),
      ...
      // 槽位缺失追问截断防线 (如果 LLM 输出指定了 missingSlots 和 clarificationMessage)
      const firstMissing = structuredRes.intents.find((i) => i.missingSlots && i.missingSlots.length > 0);
      if (firstMissing && structuredRes.clarificationMessage) {
        console.log(
          `[Structured Triage] Missing slots [${firstMissing.missingSlots?.join(', ')}], returning clarification message directly`,
        );
        return await this.handleImmediateBypass(
          state,
          'slot_clarification_structured',
          structuredRes.clarificationMessage,
          parsed,
          'structured_llm',
          parsed[0]?.confidence || 0.9,
          damageAssessment,
        );
      }

      const confidence = parsed[0]?.confidence || 0.85;
      await this.logIntentToDB(threadId, input, parsed, 'structured_llm', confidence);

      if (state.jobId) {
        agentEventEmitter.emit(`${state.jobId}:status`, {
          status: 'executing',
          node: 'triage',
          message: `用户意图识别成功！检测到核心意图: ${parsed.map((p) => p.intent).join(', ')} (置信度: ${parsed.map((p) => p.confidence.toFixed(2)).join(', ')})`,
        });
      }
      ...
      logger.error(
        { threadId, err },
        'IntentTriageEngine Step 3 structured classifier failed, falling back to general_query',
      );
      const fallbackIntents = [{ intent: 'general_query', confidence: 0.5 }];
      await this.logIntentToDB(threadId, input, fallbackIntents, 'structured_llm_fallback', 0.5);
      return {
