...
export function sanitizeTenantResponse(rawContent: string, tenantId: string): string {
  ...
  if (isSpecificMerchant) {
    sanitized = sanitized
      .replace(/\[ECOMMERCE\]/gi, brandName)
      .replace(/\[BRAND\]/gi, brandName)
      .replace(/\[STORE\]/gi, brandName)
      .replace(/\[MERCHANT\]/gi, brandName)
...
人工主管专员将在 **1 分钟内直接在本会话中为您接管服务并妥善解决**，请您稍等。给您带来的不便我们深感抱歉，感谢您的宝贵耐心！👋`;
    return { output: sanitizeTenantResponse(apology, tenantId), shortMemory };
  }
...
人工主管专员将在 **1 分钟内直接在本会话中为您接管服务并回应**，请您稍等。如您有更多细节补充，也可以直接在此留言！👋`;
      return {
        output: sanitizeTenantResponse(escalationReply, tenantId),
        shortMemory,
      };
    }
  }

  // 🛡️ [前置旁路直达响应]: 如果 Triage 阶段已经预置了精准直达答复（如规则白名单或语义缓存命中），进行商户名称清洗后直接透传
...
You are an AI Customer Support Agent representing: ${brandName} (Merchant identifier: ${tenantId}).
...
- In all user-facing sentences, refer to the store strictly by its real brand name "${brandName}". Never output raw placeholder IDs like "[ECOMMERCE]" or "[BRAND]".
- If the tool "listUserOrders" returns orders in "orders", summarize the found ${brandName} orders with their Order IDs, statuses, and amounts.
- If the tool "listUserOrders" returns an empty list or no orders found, politely inform the customer in Chinese that no order records were found under their account in ${brandName}, and invite them to provide an order number or check their login account.
...
3. If any tool returned an error or was blocked by policy (e.g. "Address modification blocked: Order ... is currently [SHIPPED]", "Order not found", or "Failed to process"), you MUST honestly and politely inform the customer in Chinese that the operation cannot be completed (for example: the parcel has already been shipped/dispatched, so the shipping address cannot be modified directly, and recommend contacting the courier or rejecting on delivery). Under NO circumstances should you state that the address was successfully changed when the tool returned an error!
4. If the tool executed successfully and returned the order details (status, carrier, etc.), you summarize them accurately.
5. If the executed tool was "listUserOrders" and returned an array of orders in "orders", provide a concise summary greeting (e.g. stating the number of orders found) and politely guide the customer to choose an order from the interactive order cards below or reply with the order ID to proceed with logistics tracking or refunds. DO NOT redundantly list out full itemized markdown details for every single order when interactive cards are already attached.
6. Keep the output professional, polite, and fully in Chinese. Refer to the store strictly as ${brandName}.
