import { db, pendingApprovals as dbPendingApprovals, getDrizzle } from 'db';
 ...
 */
export async function checkDoubleRefund(orderId: string): Promise<{ isDoubleRefund: boolean; status?: string }> {
  try {
...
export async function evaluateRefundAutoApproval(
  ...
  if (orderId && !amountStr) {
    try {
      const oRes = await db.execute('SELECT total_amount AS "totalAmount" FROM orders WHERE order_id = $1', [orderId]);
      if (oRes?.rows?.[0]) {
 ...
 */
export async function findApprovalById(approvalId: string): Promise<PendingApprovalRecord | null> {
  try {
 ...
 */
export async function findLatestApprovalByThreadId(threadId: string): Promise<PendingApprovalRecord | null> {
  try {
    const res = await db.execute(
    ...
    console.warn('[ApprovalPolicyEngine] findLatestApprovalByThreadId DB error:', err);
  }
  return null;
}
...
export async function evaluatePendingApprovalState(opts: {
    ...
    latestApproval =
      approvalsList.find((app: any) => {
        ...
        if (currentArgs.orderId && payloadArgs.orderId) {
          return String(currentArgs.orderId).trim().toLowerCase() === String(payloadArgs.orderId).trim().toLowerCase();
        }
  ...
  // 2. 无历史工单，创建新工单挂起
  if (!latestApproval) {
    const newApprovalId = crypto.randomUUID ? crypto.randomUUID() : Math.random().toString(36).substring(2, 15);
