...
export const takeScreenshot = {
  name: 'takeScreenshot',
  description: 'Takes a real physical screenshot of the specified webpage URL using local browser.',
  schema: z.object({
    url: z.string().describe('The URL of the webpage to capture, e.g. https://github.com or https://example.com'),
    fullPage: z
      .boolean()
      .optional()
      .default(false)
  ...
  execute: async ({ url, fullPage }: { url: string; fullPage?: boolean }) => {
    ...
    console.log(`[takeScreenshot] 🚀 正在直接调起您电脑本地的物理 Chrome 浏览器: ${chromePath}`);

    // Create public screenshots directory inside apps/web/public so Next.js can serve it statically
    const publicScreenshotsDir = path.resolve(__dirname, '../../../../apps/web/public/screenshots');
    try {
      if (!fs.existsSync(publicScreenshotsDir)) {
        fs.mkdirSync(publicScreenshotsDir, { recursive: true });
      }
    } catch (e) {
      console.warn('[takeScreenshot] Failed to create physical folder, skipping...', e);
    }
    ...
    try {
      ...
      console.log('[takeScreenshot] 物理导航成功，正在进行 100% 真实高清网页截图...');
      ...
      console.log(`[takeScreenshot] ✅ 截图物理磁盘落盘成功: ${physicalSavePath}`);
      await browser.close();
      ...
      return {
        ...
        message: 'Physical screenshot captured successfully using your local Chrome browser!',
      };
    } catch (err: any) {
      console.error(`❌ [takeScreenshot] 调用本地 Chrome 浏览器截图失败: ${err.message}`);
      if (browser) {
        await browser.close();
      }
      ...
      return {
        success: false,
        error: `调用本地 Chrome 失败: ${err.message}. 请确保您的电脑上安装了 Chrome 浏览器，或使用真实的 URL 进行截图测试。`,
      };
    }
  },
};
