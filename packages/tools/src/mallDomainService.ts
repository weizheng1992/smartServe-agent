...
export class MallDomainService {
  ...
  static async getUserAddresses(
    ...
    try {
      ...
      console.warn(
        "[MallDomainService.getUserAddresses] Database query error:",
        err,
      );
    }
  ...
  static async saveUserAddress(
    ...
    try {
      if (params.isDefault) {
        // 先取消其他默认地址
        await db.execute(
          "UPDATE user_addresses SET is_default = false WHERE user_id = $1 AND business_id = $2",
          [effectiveUserId, effectiveBizId],
        );
      }
      ...
      console.warn(
        "[MallDomainService.saveUserAddress] Database insert fallback:",
        err,
      );
      return {
        success: true,
   ...
   */
  static async queryProductSkus(
    params: SkuFilterParams,
  ): Promise<ToolExecutionResult> {
    ...
    try {
      ...
      if (params.color || params.size) {
        rows = rows.filter((r) => {
          ...
          if (params.color && spec.color) {
            match =
              match &&
              (spec.color.includes(params.color) ||
                params.color.includes(spec.color));
          }
          if (params.size && spec.size) {
            match =
              match &&
              (String(spec.size).includes(params.size) ||
                params.size.includes(String(spec.size)));
          }
          return match;
        });
      }
      ...
      console.warn(
        "[MallDomainService.queryProductSkus] Error querying SKUs:",
        err,
      );
    }
   ...
   */
  static async queryPackageTracking(
    ...
    try {
      ...
      console.warn(
        "[MallDomainService.queryPackageTracking] Database tracking error:",
        err,
      );
    }
      ...
      trackTimeline: [
        {
          ...
          status: "dispatching",
          description:
            "【北京市】快件已由派件员张师傅（电话：13812345678）正在为您派送，请注意接听电话",
        },
        {
   ...
   */
  static async queryProductReviews(
    ...
    try {
      ...
      console.warn(
        "[MallDomainService.queryProductReviews] Database error:",
        err,
      );
    }
   ...
   */
  static async applyAfterSale(
    params: AfterSaleParams,
  ): Promise<ToolExecutionResult> {
    ...
    // 1. 验证订单存在性与归属权
    const order = await OrderDomainService.findOrderById(
      params.orderId,
      effectiveUserId,
      effectiveBizId,
    );
    ...
    const refundAmount =
      params.refundAmount || Number(order.totalAmount || 0) || 100.0;
    ...
    try {
      ...
      console.warn(
        "[MallDomainService.applyAfterSale] Database insert failed, returning fallback ticket:",
        err,
      );
    }
  ...
  static async searchProducts(params: {
    ...
    try {
      ...
      if (query) {
        ...
        conditions.push(
          `(name ILIKE $${queryParams.length} OR description ILIKE $${queryParams.length})`,
        );
      }
      ...
      console.warn(
        "[MallDomainService.searchProducts] Database query fallback:",
        err,
      );
    }
    ...
    const mockProducts = [
      {
        ...
        stock: 58,
        description:
          "双重 Zoom Air 缓震气垫，工程网眼鞋面透气亲肤，全天候舒适缓震。",
  ...
  static async compareProducts(params: {
    ...
    const matched = all.filter(
      (p) =>
        ...
        productIds.some((id) => p.name?.includes(id)),
    );
