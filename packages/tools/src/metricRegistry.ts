...
export class MetricSemanticResolver {
  ...
  public static resolve(
    ...
    if (matches.length === 0) {
      const defaultMetric = METRIC_SEMANTIC_REGISTRY[defaultKey] || METRIC_SEMANTIC_REGISTRY.gmv;
    ...
    const conflictMetrics = conflictGroupKey
      ? Object.values(METRIC_SEMANTIC_REGISTRY).filter((m) => m.conflictGroup?.includes(conflictGroupKey))
      : [primary];

    // 仅在泛指模糊提问（如“卖得最好”、“表现最好”）且未指明具体量度（金额/销量/利润/毛利率）时标记歧义
    const genericPhrases = ['卖得好', '卖得最好', '最好', '最棒', '表现最好', '排名靠前', '头部商品'];
    ...
    const hasAmbiguity = isGeneric && !isDefinitive && conflictMetrics.length > 1;
