...
export const listUserOrders = {
  ...
  execute: async ({
    ...
    return OrderDomainService.listUserOrders(threadId, userId, businessId || tenantId);
  },
};
...
export const searchProducts = {
  ...
  schema: z.object({
    query: z.string().optional().describe('Search keywords (e.g., 跑步鞋, 连帽卫衣).'),
    category: z.string().optional().describe('Category identifier.'),
    maxPrice: z.number().optional().describe('Maximum budget filter.'),
    limit: z.number().optional().describe('Maximum number of products to return (default 4).'),
  }),
  execute: async (args: {
    query?: string;
    category?: string;
...
export const addToCart = {
  ...
  schema: z.object({
    ...
    spec: z.string().optional().describe('Selected specs (e.g. 42码 / 极夜黑).'),
  }),
  execute: async (args: {
    skuId: string;
    quantity?: number;
