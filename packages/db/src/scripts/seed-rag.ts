...
export async function seedRAGDocuments() {
  ...
  for (const doc of rawDocs) {
    ...
    const contextualSummary = await generateContextualSummary(doc.title, doc.headerPath, doc.chunkText, doc.businessId);
    ...
    const existing = await drizzle
      ...
      .from(ragDocuments)
      .where(and(eq(ragDocuments.businessId, doc.businessId), eq(ragDocuments.chunkText, doc.chunkText)))
      .limit(1);
    ...
    if (existing.length > 0) {
      ...
      console.log(`[Seed RAG] 🔄 已存在切片，更新完成: [${doc.businessId}] - [${doc.title}]`);
    } else {
      ...
      console.log(`[Seed RAG] ➕ 新增切片完成: [${doc.businessId}] - [${doc.title}]`);
    }
  }
