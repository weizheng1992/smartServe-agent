...
export const threads = pgTable(
  'threads',
  {
    ...
    unreadCount: integer('unread_count').default(0), // 坐席/用户未读计数
    tags: jsonb('tags').$type<string[]>().default([]), // 意图/业务标签 (如 ['REFUND', 'HIGH_VALUE'])
    metadata: jsonb('metadata').$type<Record<string, unknown>>().default({}), // 扩展元数据
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow(),
  },
  (table) => ({
    threadsBizStatusIdx: index('threads_biz_status_idx').on(table.businessId, table.status),
    threadsUpdatedAtIdx: index('threads_updated_at_idx').on(table.updatedAt),
  }),
);
...
export const messages = pgTable(
  'messages',
  {
    ...
    threadId: text('thread_id')
      ...
      .notNull(),
    businessId: text('business_id'), // 冗余租户字段便于快速隔离检索
    role: text('role').notNull(), // user / assistant / system / operator
    content: text('content').notNull(),
    thoughtSteps: jsonb('thought_steps'), // AI 思考链步骤快照
    toolCalls: jsonb('tool_calls'), // 工具调用入参及出参
    cards: jsonb('cards'), // 渲染的富交互卡片结构数据
    operatorInfo: jsonb('operator_info'), // 客服坐席信息快照
    timestamp: text('timestamp').notNull(),
    createdAt: timestamp('created_at').defaultNow(),
  },
  (table) => ({
    messagesThreadIdx: index('messages_thread_idx').on(table.threadId),
    messagesBizThreadIdx: index('messages_biz_thread_idx').on(table.businessId, table.threadId),
  }),
);
...
export const longMemoryFacts = pgTable(
  ...
  (table) => ({
    userScopeBizIdx: index('long_facts_user_scope_biz_idx').on(table.userId, table.scope, table.businessId),
  }),
);
...
export const episodicEvents = pgTable(
  ...
  (table) => ({
    episodicUserScopeBizIdx: index('episodic_user_scope_biz_idx').on(table.userId, table.scope, table.businessId),
  }),
);
...
export const approvalOutboxEvents = pgTable(
  ...
  (table) => ({
    outboxStatusIdx: index('approval_outbox_status_idx').on(table.status, table.createdAt),
    outboxApprovalIdx: index('approval_outbox_approval_idx').on(table.approvalId),
  }),
);
...
export const tenantConfigs = pgTable(
  ...
  (table) => ({
    bizStatusIdx: index('tenant_config_biz_status_idx').on(table.businessId, table.status),
  }),
);
...
// ============ Intent Exemplars (租户级意图 Few-Shot 样本库) ============

export const intentExemplars = pgTable(
  'intent_exemplars',
  {
    id: uuid('id').primaryKey().defaultRandom(),
    businessId: text('business_id').notNull(),
    ...
    embedding: jsonb('embedding'), // 向量特征数组 number[]
    isActive: boolean('is_active').default(true).notNull(),
    createdAt: timestamp('created_at').defaultNow(),
    updatedAt: timestamp('updated_at').defaultNow(),
  },
  (table) => ({
    exemplarBizIdx: index('intent_exemplars_biz_idx').on(table.businessId, table.intentName),
  }),
);
