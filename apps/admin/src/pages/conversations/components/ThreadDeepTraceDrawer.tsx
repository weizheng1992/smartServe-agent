...
export function ThreadDeepTraceDrawer({ isOpen, onClose, conversation, onUpdated }: ThreadDeepTraceDrawerProps) {
  ...
  const handleUpdateStatus = async (newStatus: 'active' | 'resolved') => {
    ...
    try {
      await conversationsApi.updateStatus(
        ...
        {
          status: newStatus,
          assignedOperatorId: newStatus === 'active' ? 'op_admin_01' : undefined,
        },
        conversation.businessId,
      );
      onUpdated?.();
  ...
  return (
    ...
    >
      <Tabs value={activeTab} onValueChange={(val) => setActiveTab(val as any)} className="w-full flex flex-col h-full">
        {/* Tab 导航 */}
        ...
        <TabsContent value="dialogue" className="space-y-4 mt-4 flex-1">
          <div className="bg-slate-50/80 border border-slate-200/80 rounded-2xl p-4 space-y-4 max-h-[500px] overflow-y-auto">
            {isLoadingMessages ? (
              ...
              <div className="text-center py-10 text-xs text-slate-400">暂无历史对话消息</div>
            ) : (
              messages.map((msg) => (
                <div
                  key={msg.id}
                ...
                >
                  ...
                  <div className="flex items-center gap-1.5 px-1">
                    ...
                    <span className="text-[10px] text-slate-400 font-mono">
                      {msg.timestamp ? new Date(msg.timestamp).toLocaleString('zh-CN') : conversation.updatedAt}
                    </span>
                  </div>
                  ...
                  {msg.role === 'assistant' && msg.toolCalls && (
                    <div className="w-full max-w-2xl bg-amber-50/70 border border-amber-200/80 rounded-xl p-2.5 text-xs text-amber-900 space-y-1 font-mono mb-1">
                      ...
                      {msg.toolCalls.map((t, idx) => (
                        ...
                        >
                          <div>
                            <span className="text-slate-500">入参:</span> {JSON.stringify(t.args)}
                          </div>
                          {t.result && (
                            <div>
                              <span className="text-slate-500">结果:</span> {JSON.stringify(t.result)}
                            </div>
                          )}
                        </div>
                      ))}
          ...
          <div className="p-3.5 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
            <div className="flex items-center justify-between text-xs">
              ...
              <span className="text-[11px] text-slate-400">支持 Markdown 格式与快捷发送</span>
            </div>
        ...
        <TabsContent value="metrics" className="space-y-4 mt-4">
          ...
          <div className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-3">
            <div className="text-xs font-semibold text-slate-700">会话上下文档案元数据</div>
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="p-3 bg-white rounded-xl border border-slate-200/80">
                <span className="text-slate-400 text-[11px]">会话 Thread ID:</span>
                <div className="font-mono font-semibold text-slate-800 mt-0.5">{conversation.threadId}</div>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200/80">
                <span className="text-slate-400 text-[11px]">用户 User ID:</span>
                <div className="font-mono font-semibold text-slate-800 mt-0.5">{conversation.userId}</div>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200/80">
                <span className="text-slate-400 text-[11px]">归属租户 / 业务域:</span>
                <div className="font-semibold text-slate-800 mt-0.5">
                  {conversation.businessId.toUpperCase()} (SaaS 租户)
                </div>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200/80">
                <span className="text-slate-400 text-[11px]">接入渠道 (Channel):</span>
                <div className="font-semibold text-slate-800 mt-0.5">{conversation.channel}</div>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200/80">
                <span className="text-slate-400 text-[11px]">最新更新时间:</span>
                <div className="font-mono text-slate-700 mt-0.5">{conversation.updatedAt}</div>
              </div>
              <div className="p-3 bg-white rounded-xl border border-slate-200/80">
                <span className="text-slate-400 text-[11px]">自动驾驶解决状态:</span>
                <div className="font-semibold text-slate-800 mt-0.5">
                  {conversation.status === 'waiting_approval' ? 'HITL 人工协同介入中' : 'Autopilot 智能自主完结'}
                </div>
              </div>
            </div>
          </div>
