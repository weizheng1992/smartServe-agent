    ...
    <DetailDrawer
      ...
      badge={
        conversation.status === 'waiting_approval' ? (
          <span className="px-2 py-0.5 text-xs font-semibold bg-amber-100 text-amber-800 rounded-full">待风控审批</span>
        ) : (
        ...
        <div className="flex items-center gap-2 border-b border-slate-200 pb-2">
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors cursor-pointer ${
              activeTab === 'dialogue' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
            1. 真实对话记录
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-xs font-semibold rounded-lg transition-colors cursor-pointer ${
              activeTab === 'trace' ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-slate-100'
            }`}
          >
