...
export function GuardrailFormModal({
  ...
  return (
    <FormModal
      ...
      onClose={onClose}
      onSubmit={onSubmit}
      title={isCreate ? '添加安全合规围栏规则' : `编辑安全规则: ${selectedItem?.ruleName}`}
      subtitle="定义输入过滤、输出脱敏或 SQL 沙箱防御策略"
    >
      <div className="space-y-3.5">
        <div>
          <Label className="block text-xs font-semibold text-slate-700 mb-1">规则名称</Label>
          <Input
            type="text"
            required
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label className="block text-xs font-semibold text-slate-700 mb-1">防护类型</Label>
            <select
            ...
            >
              <option value="sensitive_keyword">敏感信息与隐私 (Privacy)</option>
              <option value="sql_injection">SQL 注入防线 (SQL Injection)</option>
              <option value="prompt_leakage">Prompt 防越狱/防泄露</option>
            </select>
          </div>
          <div>
            <Label className="block text-xs font-semibold text-slate-700 mb-1">拦截响应动作</Label>
            <select
        ...
        <div>
          <Label className="block text-xs font-semibold text-slate-700 mb-1">匹配正则表达式 / 关键词</Label>
          <Textarea
            required
            rows={3}
            ...
            className="w-full text-xs bg-slate-50 border-slate-200 font-mono"
          />
        </div>
      </div>
    </FormModal>
