...
import { ApprovalContextDrawer, CheckCircle2, HumanChatModal, PendingApprovalCard, ShieldAlert } from 'ui';
...
interface PendingApprovalsProps {
  ...
  handleApprovalAction: (approvalId: string, action: 'approve' | 'reject', reason?: string) => Promise<void>;
...
export function PendingApprovals({
  ...
  const [inspectingApproval, setInspectingApproval] = useState<Approval | null>(null);
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {pendingApprovals.map((approval) => (
            <PendingApprovalCard
              ...
              onReject={(id) => handleApprovalAction(id, 'reject', rejectionReasons[approval.id])}
