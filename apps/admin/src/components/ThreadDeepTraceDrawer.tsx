...
export function ThreadDeepTraceDrawer({ isOpen, onClose, timeline, selectedMerchant }: ThreadDeepTraceDrawerProps) {
  const [activeRightTab, setActiveRightTab] = useState<'decision_flow' | 'tools_trace' | 'rag_facts' | 'billing'>(
    'decision_flow',
  );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-sm font-bold text-slate-100">会话全景决策与认知深度透视 (Cognitive Trace)</h2>
                ...
                <span className="text-xs font-mono text-slate-400">ID: {thread.threadId}</span>
              </div>
              <p className="text-[11px] text-slate-500">
                还原终端用户视角对话并穿透 LangGraph 节点时序、Tool 入参出参、RAG 向量召回及 Token 计费
              </p>
            </div>
          </div>
          ...
          <div className="col-span-12 lg:col-span-5 border-r border-slate-800 flex flex-col bg-slate-950/40">
            <div className="px-4 py-3 border-b border-slate-800/80 bg-slate-900/40 flex items-center justify-between">
              <span className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                <MessageSquare className="w-3.5 h-3.5 text-indigo-400" /> 用户原貌对话回放
              </span>
              <span className="text-[10px] text-slate-500">共 {messages.length} 条消息</span>
            </div>
                ...
                return (
                  <div key={msg.id || idx} className={`flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                    <div className="text-[10px] text-slate-500 mb-1 px-1 flex items-center gap-1">
                      <span className="font-semibold">{isUser ? '买家' : isOperator ? '人工坐席' : '智能 Agent'}</span>
            ...
            <div className="flex-1 overflow-y-auto p-5">
              ...
              {activeRightTab === 'decision_flow' && (
                <div className="space-y-4">
                  <div className="text-xs text-slate-400">智能体状态图经历以下节点完成状态流转并合成最终响应：</div>
                        ...
                        <div className="p-3.5 rounded-xl bg-slate-950/60 border border-slate-800 space-y-1.5">
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-[10px] font-mono text-slate-500">{node.duration}</span>
                            </div>
                          </div>
              ...
              {activeRightTab === 'tools_trace' && (
                <div className="space-y-4">
                  <div className="text-xs text-slate-400">本会话中 Agent 调用的标准化工具入参与出参审计：</div>
                  {mockToolTraces.map((t) => (
                    <div key={t.tool} className="p-4 rounded-xl bg-slate-950/80 border border-slate-800 space-y-3">
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <Wrench className="w-4 h-4 text-amber-400" />
                          <span className="text-xs font-bold text-slate-200 font-mono">{t.tool}</span>
                      ...
                      <div className="space-y-2">
                        ...
                        <div>
                          <span className="text-[10px] font-bold text-slate-500 uppercase">Execution Output:</span>
              ...
              {activeRightTab === 'rag_facts' && (
                <div className="space-y-4">
                  <div className="text-xs text-slate-400">
                    基于 pgvector 混合检索 (BM25 + RRF) 命中的租户私有知识库切片：
                  </div>
                  {mockRagFacts.map((fact) => (
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-indigo-300">{fact.title}</span>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-mono text-emerald-400">Score: {fact.score}</span>
                          <Badge className="bg-slate-800 text-slate-400 text-[9px]">{fact.category}</Badge>
                        </div>
                      </div>
                ...
                <div className="space-y-4">
                  <div className="grid grid-cols-3 gap-3">
                    <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                      <div className="text-[10px] text-slate-500 uppercase font-bold">Input Tokens</div>
                      <div className="text-base font-bold text-slate-200 font-mono mt-1">1,420</div>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                      <div className="text-[10px] text-slate-500 uppercase font-bold">Output Tokens</div>
                      <div className="text-base font-bold text-slate-200 font-mono mt-1">388</div>
                    </div>
                    <div className="p-3 rounded-xl bg-slate-950/60 border border-slate-800">
                      <div className="text-[10px] text-slate-500 uppercase font-bold">Total Cost (USD)</div>
                      <div className="text-base font-bold text-emerald-400 font-mono mt-1">$0.0054</div>
                    </div>
                  </div>

                  <div className="p-4 rounded-xl bg-slate-950/40 border border-slate-800 text-xs text-slate-400 space-y-2">
                    <div className="font-bold text-slate-300">SaaS 计费规则说明：</div>
