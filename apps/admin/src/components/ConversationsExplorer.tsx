...
const normalizeCards = (cards?: any[]) => {
  ...
  return cards.map((c) => {
    ...
    if (type === "RefundConfirmationCard" || type === "refundConfirmation")
      type = "refund_confirmation";
    if (type === "TrackingTimeline" || type === "trackingTimeline")
      type = "tracking_timeline";
    if (type === "OrderCard" || type === "orderCard") type = "order_card";
    if (type === "DamageAssessmentCard" || type === "damageAssessment")
      type = "damage_assessment";
    if (type === "ProductRankingCard" || type === "productRanking")
      type = "product_ranking";
    if (type === "QuickReplies" || type === "quickReplies")
      type = "quick_replies";
    return {
      type: type as any,
...
export function ConversationsExplorer({
  ...
  const fetchConversations = useCallback(async () => {
    ...
    try {
      ...
      try {
        res = await fetch(
          `http://localhost:4000/api/conversations?${params.toString()}`,
          {
  ...
  const loadTimeline = async (threadId: string) => {
    ...
    try {
      ...
      try {
        res = await fetch(
          `http://localhost:4000/api/conversations/${threadId}?tenantId=${selectedMerchant}`,
          {
            headers: { "x-tenant-id": selectedMerchant },
          },
        );
        ...
        <div className="lg:col-span-6 space-y-3">
          ...
          {conversations.length === 0 ? (
            ...
            conversations.map((conv) => {
              const isSelected =
                selectedTimeline?.thread.threadId === conv.threadId;
              return (
                <div
                  key={conv.threadId}
              ...
              <div className="flex items-center gap-2">
                ...
                {selectedTimeline && onSelectForLiveDesk && (
                  <Button
                    size="sm"
                    onClick={() =>
                      onSelectForLiveDesk(selectedTimeline.thread.threadId)
                    }
                      ...
                      >
                        ...
                        {msg.cards && msg.cards.length > 0 && (
                          <div className="mt-3 space-y-2">
                            <RichCardRenderer
                              cards={normalizeCards(msg.cards)}
                            />
                          </div>
                        )}
                      </div>
