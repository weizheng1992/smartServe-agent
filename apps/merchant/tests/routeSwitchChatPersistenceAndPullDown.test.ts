...
describe('🧪 TDD: Merchant Route Switch Chat Persistence & Pull-Down History Loading', () => {
  ...
  it('2. [Red -> Green] 支持在当前会话中下拉/加载更早的完整历史会话流 (all_history=true / load_older)', async () => {
    ...
    const hasCouponMsg = allJson.allHistoricalMessages.some((m: any) => m.content.includes('优惠券'));
    const hasReturnMsg = allJson.allHistoricalMessages.some((m: any) => m.content.includes('7天无理由'));
