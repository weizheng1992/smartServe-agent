...
describe('🛍️ Merchant Route Pages & Route-Aware Greetings Suite', () => {
  ...
  it('2. 应该能通过 /api/store/products/[id] 获取单个 SPU/Product 详情', async () => {
    ...
    const detailReq = new NextRequest(`http://localhost:3005/api/store/products/${targetId}`);
  ...
  it('3. 应该能通过 /api/store/orders 支持 customerId/userId 查询与单笔订单详情', async () => {
    ...
    const listReq = new NextRequest('http://localhost:3005/api/store/orders?customerId=CUST-8801');
    ...
    if (listJson.orders.length > 0) {
      ...
      const detailReq = new NextRequest(`http://localhost:3005/api/store/orders/${orderId}`);
