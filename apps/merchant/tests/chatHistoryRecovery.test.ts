...
describe('Chat History Recovery by User and Thread', () => {
  it('should recover previous chat messages for CUST-8801 even if a new browser/fresh thread opens', async () => {
    ...
    const res = await fetch(`http://localhost:3005/api/store/chat/messages?userId=CUST-8801&tenantId=aurora`);
    ...
    const found = listRes.items.find((item) => item.userId === 'CUST-8801' || item.threadId.includes('CUST-8801'));
    expect(found).toBeDefined();
    expect(found?.threadId).toBe(testThreadId);

    const timeline = await ConversationRepository.getConversationTimeline(testThreadId, 'aurora');
