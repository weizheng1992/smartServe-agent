...
describe('🧪 TDD: Merchant Storefront Chat History & Live Desk Takeover Sync', () => {
  ...
  it('1. [Red -> Green] 客户端发送第一条消息后，应自动创建会话并持久化 threadId 与消息记录', async () => {
    ...
    // 验证数据库物理持久化
    const timeline = await ConversationRepository.getConversationTimeline(testThreadId, tenantId);
    expect(timeline).toBeDefined();
    expect(timeline?.messages.length).toBeGreaterThanOrEqual(1);
    expect(timeline?.messages[0].content).toContain('我想查一下我最近的订单物流');
  }, 30000);
  ...
  it('3. [Red -> Green] 管理员在 Admin 后台发送人工回复后，商城客户端应能同步拉取到人工客服消息', async () => {
    ...
    const replyText = '您好！我是极光潮品人工客服主管，已为您核实到物流正在派送中。';
    const adminReplyReq = new NextRequest('http://localhost:3005/api/admin/approvals', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
  ...
  it("4. [Red -> Green] 发送'你好'问候语后，消息历史中不应产生重复的用户消息记录 (用户只发了1次'你好')", async () => {
    ...
    const sendGreetingReq = new NextRequest('http://localhost:3005/api/store/chat', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        message: '你好',
    ...
    expect(historyJson.success).toBe(true);
    const userMessages = historyJson.messages.filter((m: any) => m.role === 'user' && m.content === '你好');
  ...
  it('6. [Red -> Green] 即使租户下存在大量其他会话，指定 userId 查询时仍能准确检索到该用户的历史会话列表', async () => {
    ...
    const matchedThread = queryJson.userThreads.find((t: any) => t.threadId === customThread);
