...
import { GET as getOrderDetailRoute } from '../app/api/store/orders/[orderId]/route';
import { GET as getOrdersRoute, POST as postOrdersRoute } from '../app/api/store/orders/route';
...
describe('🛍️ Merchant Storefront E-Commerce Complete Flow (Cart / Orders / Address / Tracking)', () => {
  ...
  describe('1. 地址簿管理与默认地址设定 (Address Management)', () => {
    it('1.1 应该能获取当前顾客的多条收货地址', async () => {
      const addresses = await MerchantDomainService.getCustomerAddresses(TEST_USER);
    ...
    it('1.2 应该能新增一条收货地址并支持设为默认地址', async () => {
      ...
      const result = await MerchantDomainService.saveCustomerAddress(TEST_USER, newAddr);
      expect(result.success).toBe(true);
      expect(result.address?.id).toBeDefined();

      // 验证新增后并重新读取地址列表
      const updatedList = await MerchantDomainService.getCustomerAddresses(TEST_USER);
    ...
    it('1.3 通过 HTTP API GET & POST /api/store/addresses 操作地址簿', async () => {
      // 1. GET 请求
      const getReq = new NextRequest(`http://localhost:3005/api/store/addresses?userId=${TEST_USER}`);
      ...
      // 2. POST 新增请求
      const postReq = new NextRequest('http://localhost:3005/api/store/addresses', {
        method: 'POST',
  ...
  describe('2. 购物车批量多商品结算与扣减库存 (Cart Checkout)', () => {
    ...
    it('2.1 应该支持将多个不同规格商品从购物车批量下单并生成订单明细', async () => {
      ...
      const updatedSku1 = afterProducts[0].skus.find((s) => s.skuCode === sku1.skuCode);
      const updatedSku2 = afterProducts[1].skus.find((s) => s.skuCode === sku2.skuCode);
    ...
    it('2.2 通过 HTTP API POST /api/store/orders 支持购物车合并下单', async () => {
      ...
      const postReq = new NextRequest('http://localhost:3005/api/store/orders', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          customerId: TEST_USER,
  ...
  describe('3. 订单详情、多状态检索与可视化物流轨迹 (Order Detail & Tracking)', () => {
    ...
    it('3.1 应该能通过 GET /api/store/orders 按状态过滤订单', async () => {
      const req = new NextRequest(`http://localhost:3005/api/store/orders?userId=${TEST_USER}&status=SHIPPED`);
    ...
    it('3.2 应该能通过 GET /api/store/orders/[orderId] 拉取完整订单详情与物流节点时间线', async () => {
      const req = new NextRequest(`http://localhost:3005/api/store/orders/${shippedOrderId}`);
      const res = await getOrderDetailRoute(req, {
        params: Promise.resolve({ orderId: shippedOrderId }),
      });
      expect(res.status).toBe(200);
      const json = await res.json();
      expect(json.success).toBe(true);
