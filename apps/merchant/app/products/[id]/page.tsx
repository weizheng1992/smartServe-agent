  ...
  const handleAddToCart = () => {
    ...
    try {
      const existingCart = JSON.parse(
        localStorage.getItem("aurora_store_cart") || "[]",
      );
      const idx = existingCart.findIndex(
        (it: any) => it.sku.skuCode === selectedSku.skuCode,
      );
      if (idx >= 0) {
          ...
          <div className="lg:col-span-7 flex flex-col justify-between space-y-6">
            <div className="space-y-4">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-200 flex items-baseline space-x-3">
                <span className="text-emerald-700 font-extrabold text-3xl">
                ...
                <span className="text-xs text-slate-500 ml-auto">
                  库存:{" "}
                  <strong className="text-emerald-700">{currentStock}</strong>{" "}
                  件
                </span>
              </div>
              ...
              <div className="flex items-center space-x-4 pt-2">
                ...
                <div className="flex items-center border border-slate-200 rounded-lg overflow-hidden bg-white shadow-2xs">
                  ...
                  <span className="px-4 py-1 text-xs font-bold text-slate-800 min-w-8 text-center">
                    {buyQuantity}
                  </span>
                  <button
                    type="button"
                    onClick={() =>
                      setBuyQuantity((q) => Math.min(currentStock, q + 1))
                    }
              ...
              <table className="w-full text-left text-xs">
                <tbody className="divide-y divide-slate-100">
                  {Object.entries(product.specs).map(([key, value]) => (
                    <tr key={key}>
                      <td className="p-3 bg-slate-50 text-slate-500 font-semibold w-36">
                        {key}
                      </td>
