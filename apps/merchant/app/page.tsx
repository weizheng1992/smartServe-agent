...
export default function StorefrontPage() {
  ...
  useEffect(() => {
    ...
    // 初始化获取或生成 ThreadId 并拉取历史记录
    const storedThreadId = localStorage.getItem(
      "merchant_storefront_thread_id",
    );
    const effectiveThreadId =
      storedThreadId || `merchant_thread_CUST-8801_${Date.now()}`;
    if (!storedThreadId) {
      localStorage.setItem("merchant_storefront_thread_id", effectiveThreadId);
    }
    setChatThreadId(effectiveThreadId);

    const loadChatHistory = async () => {
      try {
        const res = await fetch(
          `/api/store/chat/messages?threadId=${effectiveThreadId}&userId=CUST-8801&businessId=aurora`,
        );
        const data = await res.json();
        if (
          data.success &&
  ...
  // 当客服弹窗打开时，每 3 秒轮询同步人工客服消息与最新对话
  useEffect(() => {
    if (!showChatModal || !chatThreadId) return;

    const pollInterval = setInterval(async () => {
      try {
        const res = await fetch(
          `/api/store/chat/messages?threadId=${chatThreadId}&userId=CUST-8801&businessId=aurora`,
        );
        const data = await res.json();
        if (
          data.success &&
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      const resp = await fetch("/api/store/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          message: userMsg,
