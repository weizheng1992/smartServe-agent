...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  return (
    <Sheet open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <SheetContent
        side="right"
        className="w-full max-w-md p-0 flex flex-col bg-white"
      >
        {/* 抽屉顶部 Header */}
                ...
                >
                  ...
                  <div className="mt-3.5">
                    <Checkbox
                      ...
                      onCheckedChange={() => onToggleSelect(item.skuCode)}
                    />
                  </div>
        ...
        {cart.length > 0 && (
          <div className="p-4 border-t border-slate-200 bg-white space-y-3">
            ...
            <Button
              ...
              onClick={onCheckout}
              disabled={
                selectedItems.length === 0 || isCheckingOut || !selectedAddress
              }
            ...
            >
              {isCheckingOut ? (
                ...
                <span>立即结算 ({selectedItems.length})</span>
              )}
            </Button>
          </div>
        )}
