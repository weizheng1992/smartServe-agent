...
export const CartDrawer: React.FC<CartDrawerProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden">
      {/* 遮罩背景 */}
      <div
        className="absolute inset-0 bg-slate-900/60 backdrop-blur-xs transition-opacity animate-fade-in cursor-pointer"
        onClick={onClose}
      />
        ...
        <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col">
          ...
          <div className="p-4 sm:p-5 border-b border-slate-200 flex items-center justify-between bg-slate-50/80">
            <div className="flex items-center space-x-2">
              ...
              <h2 className="text-lg font-bold text-slate-900">我的购物车 ({cart.length})</h2>
            </div>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-5 space-y-4">
            ...
            <div className="bg-emerald-50 border border-emerald-200/80 rounded-xl p-3.5 flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="text-xs font-medium text-emerald-700 hover:text-emerald-900 bg-white border border-emerald-300 px-2.5 py-1 rounded-md shadow-2xs hover:bg-emerald-50 shrink-0 transition"
              >
                切换
              </button>
            </div>
                ...
                {cart.map((item) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={() => onToggleSelect(item.skuCode)}
                      className="mt-3.5 w-4 h-4 text-emerald-600 rounded border-slate-300 focus:ring-emerald-500 cursor-pointer"
                    />
                      ...
                      <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex items-center border border-slate-200 rounded-lg bg-white overflow-hidden shadow-2xs">
                          <button
                            ...
                            className="px-2 py-0.5 text-xs text-slate-600 hover:bg-slate-100 disabled:opacity-30 disabled:hover:bg-transparent"
                          >
                            -
                          </button>
          ...
          {cart.length > 0 && (
            <div className="p-4 border-t border-slate-200 bg-white space-y-3">
              ...
              <button
                ...
                className="w-full py-3 bg-emerald-600 hover:bg-emerald-500 text-white font-bold rounded-xl shadow-md transition-all flex items-center justify-center space-x-2 disabled:bg-slate-300 disabled:cursor-not-allowed cursor-pointer"
              >
