...
export async function GET(req: NextRequest) {
  try {
    ...
    const includeOlder = searchParams.get('includeOlder') === 'true' || searchParams.get('allHistory') === 'true';

    let allHistoricalMessages: any[] | undefined = undefined;
    if (includeOlder && userThreads.length > 0) {
      const allMsgs: any[] = [];
      for (const t of userThreads) {
        const tl = await ConversationRepository.getConversationTimeline(t.threadId, tenantId);
