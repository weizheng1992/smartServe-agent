...
describe('全链路多轮对话与状态机自动化流程测试 (Chat Dialogue Scenario & State Machine Suite)', () => {
  ...
  test('【场景二：HITL 转人工全生命周期隔离】转人工 -> 等待中追问被拦截直接存库 -> 人工回复解决 -> 恢复智能客服', async () => {
    ...
    const dispatchRes = await dispatchChatRequest({
      message: userFollowUp,
      threadId,
      userId,
      businessId,
    });
    expect(dispatchRes.isHumanActive).toBe(true);
    expect(dispatchRes.jobId).toBeUndefined();
    ...
    await drizzle!
      ...
      .where(eq(pendingApprovals.id, humanTicket.id));
