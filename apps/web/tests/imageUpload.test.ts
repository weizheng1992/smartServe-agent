...
describe('📸 Image Upload API Endpoint', () => {
  ...
  it('应正常保存合规的 PNG/JPG 图片并返回访问 URL', async () => {
    const formData = new FormData();
    const fakeImage = new File([new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10])], 'broken_shoe.png', {
      type: 'image/png',
    });
  ...
  it('应正常支持并保存 GIF 图片', async () => {
    const formData = new FormData();
    const fakeGif = new File([new Uint8Array([71, 73, 70, 56, 57, 97])], 'damage_anim.gif', {
      type: 'image/gif',
    });
