...
export async function GET(req: NextRequest) {
  try {
    ...
    try {
      ...
      console.warn('[API /api/messages] Failed to resolve thread businessId:', tErr);
    }
    ...
    return NextResponse.json({
      ...
      messages: messages.map((m) => ({
        id: m.id,
        role: m.role,
        content: m.role === 'assistant' && businessId ? sanitizeTenantResponse(m.content, businessId) : m.content,
