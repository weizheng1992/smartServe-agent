...
export async function GET(req: NextRequest) {
  try {
    ...
    const totalCost = rows.reduce((sum, r) => sum + Number(r.calculatedCostUsd || r.calculated_cost_usd || 0), 0);
    const totalTokens = rows.reduce((sum, r) => sum + Number(r.totalTokens || r.total_tokens || 0), 0);
    const totalLatency = rows.reduce((sum, r) => sum + Number(r.avgLatencyMs || r.avg_latency_ms || 0), 0);
