...
interface UseApprovalsProps {
  ...
  loadHistory: (threadId: string, force?: boolean) => Promise<void>;
...
export function useApprovals({
  ...
  const [activeChatApproval, setActiveChatApproval] =
    useState<PendingApprovalRecord | null>(null);

  const lastApprovalsStateRef = useRef<Record<string, string>>({});
  const { executeApprovalAction, executeHumanReplyAction } =
    useApprovalMachine();
        ...
        if (data.success && data.approvals) {
          ...
          if (activeThreadId) {
            ...
            const isHumanActive = activeApprovals.some(
              (a: PendingApprovalRecord) =>
                a.actionType?.includes("human") ||
                a.actionType?.includes("escalat"),
            );
            if (isHumanActive) {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (data?.jobId) {
          ...
          setMessages((prev) =>
            prev.filter((m) => m.jobId !== "resume-pending-job"),
          );
