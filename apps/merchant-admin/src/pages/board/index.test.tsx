...
const RESULT_FRAME = [
  { event: 'result', data: { metric: 'volume', unit: '件', caliber: '口径', rows: [{ a: 1, b: 2 }, { a: 2, b: 4 }], cards: [{ type: 'table', title: 't', columns: [{ key: 'a', label: 'a' }, { key: 'b', label: 'b' }], rows: [{ a: 1, b: 2 }], caliber: 'c' }] } },
];
...
afterEach(() => { try { if (document.fullscreenElement) void document.exitFullscreen(); } catch {} });

describe('BoardPage(常驻看板)', () => {
  it('挂载即按钉卡重放 api.ask,并渲染结果卡', async () => {
    render(<MemoryRouter><BoardPage /></MemoryRouter>);
    await waitFor(() => expect(askMock).toHaveBeenCalledWith(
      '品类GMV排行', expect.objectContaining({ route: '/analytics', selection: [] }),
    ));
