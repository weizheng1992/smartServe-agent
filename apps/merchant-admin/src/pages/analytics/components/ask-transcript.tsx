...
/** 问答流水:用户气泡 / 折线卡 / 表格卡 / clarify 反问 / 诚实拒绝与错误。 */
              ...
              {f.event === 'clarify' && (
                <div className="mt-2 flex flex-wrap gap-2">
                  {(f.data.options || []).map((o: any, j: number) => (
                    <button key={j} type="button" className="rounded-lg border border-zinc-300 px-3 py-1.5 text-xs" onClick={() => onAsk(`按${o.label}的商品排行`)}>
                      {o.label}
                    </button>
                  ))}
                </div>
