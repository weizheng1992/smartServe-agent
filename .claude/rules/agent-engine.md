...
本工作区是整个平台的核心中枢，负责 LangGraph DAG 状态图调度、Skills 技能分发、四象限记忆体系、双层客户画像隔离、多模态视觉定责、Contextual RAG 检索、审批门禁与 Temporal 分布式工作流。
...
- **技能基类 (`BaseSkill`)**：所有业务技能（如 `OrderRefundSkill`）必须继承 `BaseSkill`，提供统一的元数据、依赖工具声明、SOP 策略以及多阶段执行管道（`validate` ➔ `preExecute` ➔ `execute` ➔ `postExecute`）。
- **执行器调度 (`stepExecutionEngine.ts`)**：
  - 优先通过 `SkillsRegistry` 检索匹配的 Skill 实例执行业务逻辑；
  - 若无特定 Skill，则回退至标准 Tool 工具分发执行。
...
### 1.5 Contextual RAG 知识检索 (`contextualRag.ts`)
...
### 1.6 审批门禁与事务发件箱 (`approvalGatekeeper.ts` & `approvalOutboxWorker.ts`)
