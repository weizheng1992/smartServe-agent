...
- **Planner 约束**：`plannerNode` 根据 Nike 的 30天政策，能精准规划出 _“30天试穿内退货，无需回退到 planner 说明不符”_ 的步骤；
...
当物理数据库处于 offline 或本地以 `FakePool` (`packages/db/src/fakePool.ts`) 运行时，系统将静默容错，**无缝降级到 Local Map 模糊关键词匹配检索，依然能输出完全一致的 SaaS 隔离的模拟 RAG 结果**，保障了整个单体环境在任何恶劣测试条件下的 100% 完备运行：

```typescript
// packages/db/src/fakePool.ts 内置了 RAG 物理 SQL query 的物理静态仿真
...
_文档编写日期：2026-07-27_
_检索架构：Contextual-Embedding (Gemini-3.5-Flash & Text-Embedding)_
